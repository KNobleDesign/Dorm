...
export function exportDashboardToExcel({
  ...
  const roomRows = rooms.map((r, idx) => {
    ...
    const paymentStatusText = r.isPaid ? 'ชำระแล้ว' : (liability > 0 ? 'มียอดค้างเดิม' : 'รอชำระ');
  ...
  const financialOverviewRows = [
    ...
    { 'หัวข้อสรุปภาพรวม': 'ยอดรอชำระงวดนี้ (Pending Revenue)', 'รายละเอียด': `${pendingRevenue.toLocaleString()} บาท (${unpaidCount} ห้อง)` },
    { 'หัวข้อสรุปภาพรวม': 'หนี้ค้างชำระเดิมยกมา (Total Arrears)', 'รายละเอียด': `${rooms.reduce((s, r) => s + (r.previousBalance || 0), 0).toLocaleString()} บาท` },
    { 'หัวข้อสรุปภาพรวม': '--------------------------------', 'รายละเอียด': '--------------------------------' },
...
export function exportRoomsToCsv({
  ...
  const rows = rooms.map((r, idx) => {
    ...
    return [
      ...
      escapeCsv(r.isPaid ? 'ชำระแล้ว' : (liability > 0 ? 'มียอดค้างเดิม' : 'รอชำระ'))
    ].join(',');
  });
