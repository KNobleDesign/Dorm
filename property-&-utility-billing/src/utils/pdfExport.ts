...
export async function captureElementToDataUrl(
  ...
  try {
    ...
    console.warn('html-to-image standard capture failed, trying with skipFonts:', err1);
  }

  // Tier 2: html-to-image with skipFonts (avoids external font network fetch errors)
  try {
    const dataUrl = await htmlToImage.toPng(element, {
      pixelRatio,
      backgroundColor: '#ffffff',
    ...
    console.warn('html-to-image with skipFonts failed, trying html2canvas-pro:', err2);
  }

  // Tier 3: html2canvas-pro fallback
  try {
    ...
    console.error('All PDF image capture methods failed:', err3);
