...
export async function captureElementToDataUrl(
  ...
  const targetWidth = options.width || (element.offsetWidth > 0 ? element.offsetWidth : 794);
  const targetHeight = options.height || (element.offsetHeight > 0 ? element.offsetHeight : 1123);
  const pixelRatio = options.scale || 2;

  // Create an isolated sandbox attached directly to document.body
  // This guarantees exact dimensions, zero horizontal clipping by responsive containers,
  // zero interference from overflow-x-hidden, and exact coordinate mapping (left: 0, top: 0).
  ...
  clone.style.padding = element.style.padding || '12mm 15mm';
  ...
  try {
    // Wait a brief tick for clone rendering and style resolution
    await new Promise((resolve) => setTimeout(resolve, 60));

    // Tier 1: html-to-image with skipFonts: true
    try {
      ...
      console.warn('html-to-image clone capture failed, trying html2canvas-pro fallback:', err1);
    }

    // Tier 2: html2canvas-pro on clone with exact coordinates
    try {
      ...
      console.warn('html2canvas on clone failed, trying direct element capture:', err2);
    }

    // Tier 3: Fallback on original element
    ...
    return directCanvas.toDataURL('image/png', 0.95);
  } finally {
