...
import { captureElementToDataUrl } from '../utils/pdfExport';
...
export const DashboardPdfExportModal: React.FC<DashboardPdfExportModalProps> = ({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const imgData = await captureElementToDataUrl(printRef.current, {
        scale: 2,
        width: 1000,
      });
      ...
      const elHeight = printRef.current.offsetHeight || 1000;
      const elWidth = printRef.current.offsetWidth || 1000;
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2.5">
                ...
                <div className="p-3 bg-amber-50/80 border border-amber-200 rounded-xl">
                  <div className="text-[10px] text-amber-800 font-semibold">รอชำระงวดนี้ (Pending Payment)</div>
                  <div className="text-sm font-bold text-amber-900 mt-0.5">฿{pendingRevenue.toLocaleString()}</div>
                  <div className="text-[9px] text-amber-700 mt-0.5">{unpaidRooms.length} ห้องรอชำระ</div>
                </div>
            ...
            {unpaidRooms.length > 0 && (
              <div>
                <h2 className="text-xs font-bold text-amber-900 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  ...
                  <span>3. รายการห้องที่รอชำระเงิน (Pending Payment Rooms - {unpaidRooms.length} ห้อง)</span>
                </h2>
                ...
                {unpaidRooms.length > 10 && (
                  <p className="text-[10px] text-slate-500 mt-1 italic">
                    * แสดง 10 ห้องแรก จากทั้งหมด {unpaidRooms.length} ห้องที่รอชำระ
                  </p>
                )}
              </div>
            )}
