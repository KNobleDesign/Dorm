              ...
              <div className="p-4 bg-amber-50/80 rounded-xl border border-amber-300 space-y-3">
                ...
                <div className="pt-2 border-t border-amber-200/60">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.carryOverArrearsDefault ?? false}
                      onChange={(e) => setFormData(prev => ({ ...prev, carryOverArrearsDefault: e.target.checked }))}
                      className="w-4 h-4 text-amber-600 rounded border-gray-300 focus:ring-amber-500"
                    />
                    <div>
                      <span className="text-xs font-bold text-slate-800">ทบยอดค้างชำระไปงวดถัดไปโดยอัตโนมัติ (Carry-over Unpaid Balances)</span>
                      <p className="text-[11px] text-slate-500">ค่าเริ่มต้น: ปิด (ไม่มีค้างจ่ายเป็นค่าเริ่มต้น ทุกเดือนจะเริ่มต้นยอดค้างชำระที่ ฿0)</p>
                    </div>
                  </label>
                </div>
              </div>
