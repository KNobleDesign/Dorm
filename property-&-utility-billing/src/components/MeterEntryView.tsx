...
export const MeterEntryView: React.FC<MeterEntryViewProps> = ({
  ...
  const buildingRooms = selectedBuilding === 'all' 
    ...
    : rooms.filter(r => 
        ...
        (r.building && selectedBuilding && (
          r.building.replace(/^(บ้านเช่า|อาคาร)\s*/i, '').trim() === selectedBuilding.replace(/^(บ้านเช่า|อาคาร)\s*/i, '').trim()
        ))
      );
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const elWidth = element.offsetWidth || 1100;
      const elHeight = element.offsetHeight || 800;

      const imgData = await captureElementToDataUrl(element, {
        scale: 2,
        width: elWidth,
        height: elHeight,
      });
      ...
      const totalImgHeight = (elHeight * printWidth) / elWidth;
