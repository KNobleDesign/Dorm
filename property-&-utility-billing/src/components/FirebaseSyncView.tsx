...
interface FirebaseSyncViewProps {
  ...
  onResetToPdfData?: () => Promise<void> | void;
...
export const FirebaseSyncView: React.FC<FirebaseSyncViewProps> = ({
  ...
  // Reset all buildings & rooms strictly according to the official PDF
  const handleResetToPdf = async () => {
    if (!onResetToPdfData) return;
    const confirmed = window.confirm(
      'ต้องการรีเซ็ตข้อมูลอาคารและห้องพักทั้งหมดให้ตรงตามเอกสาร PDF (6 อาคาร 78 ห้อง) เริ่มต้นที่งวดเดือน 09 ก.ย. และส่งขึ้น Firebase หรือไม่?'
    );
    if (!confirmed) return;

    setIsResetting(true);
    try {
      await onResetToPdfData();
      showToast('รีเซ็ตข้อมูลเริ่มต้นตาม PDF (78 ห้อง, 6 อาคาร) และซิงค์ขึ้น Firebase สำเร็จแล้ว!', 'success');
      const newLog = {
        id: `reset-${Date.now()}`,
        time: new Date().toLocaleTimeString('th-TH'),
        action: 'รีเซ็ตข้อมูลทั้งหมดตาม PDF',
        status: 'ok' as const,
        detail: 'ตั้งค่าอาคาร 6 อาคาร และห้องพัก 78 ห้องเริ่มต้นงวด 09 ก.ย. ส่งขึ้น Cloud เรียบร้อย'
      };
      setSyncHistory(prev => [newLog, ...prev.slice(0, 19)]);
    } catch (e: any) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12 font-google-sans">
      ...
      <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-amber-600/10 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          {/* Quick Push / Pull / Reset buttons */}
          <div className="flex flex-wrap items-center gap-3">
            {onResetToPdfData && (
              <button
                onClick={handleResetToPdf}
                disabled={isResetting || isPushing}
                title="รีเซ็ตข้อมูลอาคาร (6 อาคาร) และห้องพัก (78 ห้อง) เริ่มต้นตามเอกสาร PDF"
                className="inline-flex items-center gap-2 px-4 py-3 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-2xl shadow-lg shadow-black/10 transition cursor-pointer disabled:opacity-50"
              >
                <RefreshCw className={`w-4 h-4 text-white ${isResetting ? 'animate-spin' : ''}`} />
                <span>{isResetting ? 'กำลังรีเซ็ตตาม PDF...' : 'รีเซ็ตตาม PDF (78 ห้อง)'}</span>
              </button>
            )}

            <button
              onClick={handlePushAll}
              disabled={isPushing || isResetting}
            ...
            <button
              ...
              disabled={isPulling || isResetting}
      ...
      {/* Guide for Firebase Console & Cloud Firestore (ตรงกับภาพหน้าจอที่เปิดอยู่) */}
      <div className="bg-amber-50/80 border-2 border-amber-300 rounded-3xl p-6 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-amber-950 flex items-center gap-2">
                คำแนะนำ: การดูข้อมูลใน Firebase Console (Cloud Firestore & Realtime Database)
              </h2>
              <p className="text-xs sm:text-sm text-amber-900/90 mt-0.5">
                ในภาพหน้าจอ Firebase Console ของคุณ คุณกำลังเปิดหน้า <strong>Cloud Firestore</strong> ซึ่งมีเมนู <strong>Realtime Database</strong> อยู่ด้านล่างถัดไป
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            <button
              onClick={handlePushAll}
              disabled={isPushing}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white text-xs sm:text-sm font-bold rounded-xl shadow-md shadow-amber-600/20 transition cursor-pointer disabled:opacity-50"
            >
              <Zap className={`w-4 h-4 ${isPushing ? 'animate-spin' : ''}`} />
              <span>{isPushing ? 'กำลังส่งข้อมูล...' : '🔥 บันทึกข้อมูลขึ้น Cloud Firestore ทันที'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-2">
          ...
          <div className="p-4 bg-white rounded-2xl border border-amber-200 space-y-2">
            <div className="flex items-center gap-2 font-bold text-slate-900 text-sm">
              ...
              <span>1. Cloud Firestore (หน้าจอที่คุณเปิดอยู่)</span>
            </div>
            <p className="text-slate-600">
              เมื่อกดปุ่ม <strong>"🔥 บันทึกข้อมูลขึ้น Cloud Firestore ทันที"</strong> ด้านบน แล้ว <strong>Refresh หน้าเว็บ Firebase</strong> จะพบ Collections:
            </p>
            <div className="flex flex-wrap gap-1.5 pt-1">
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">dormitory</span>
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">dorm_config</span>
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">buildings</span>
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">monthly_records</span>
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">expenses</span>
              <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-mono font-bold rounded">users</span>
            </div>
          </div>
          ...
          <div className="p-4 bg-white rounded-2xl border border-amber-200 space-y-2">
            <div className="flex items-center gap-2 font-bold text-slate-900 text-sm">
              ...
              <span>2. Realtime Database (เมนูด้านซ้ายล่างถัดจาก Firestore)</span>
            </div>
            <p className="text-slate-600">
              ในเมนูด้านซ้ายของ Firebase Console หากคลิกที่ <strong>"Realtime Database"</strong> จะเห็นโหนด <code className="bg-slate-100 px-1 py-0.5 rounded font-mono font-bold text-amber-800">dorm_app</code> ซึ่งใช้ในการรับส่งข้อมูลสดความเร็วสูงระดับมิลลิวินาที
            </p>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-amber-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-amber-200">
            <div>
              <h4 className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-emerald-600" />
                <span>กฎความปลอดภัย (Security Rules) ในแท็บ "Rules"</span>
              </h4>
              <p className="text-[11px] text-slate-500 mt-0.5">
                หากระบบแจ้งติดสิทธิ์ (Permission Denied) ให้ไปที่แท็บ <strong>Rules</strong> (ข้างแท็บ Data ในรูปของคุณ) แล้ววางกฎด้านล่างนี้:
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                <span>{copiedKey === 'Firestore Rules' ? 'คัดลอกแล้ว!' : 'คัดลอก Firestore Rules'}</span>
              </button>
              ...
              >
                <Copy className="w-3.5 h-3.5 text-slate-600" />
                <span>{copiedKey === 'Realtime DB Rules' ? 'คัดลอกแล้ว!' : 'คัดลอก RTDB Rules'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* GitHub & Firebase Deployment Quick Guide */}
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 rounded-3xl border border-slate-700/50 shadow-xl p-6 sm:p-8 text-white space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-700/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black tracking-tight">คู่มือการนำโปรเจกต์ขึ้น GitHub และเชื่อมต่อไปยัง Firebase</h3>
              <p className="text-xs text-slate-400 mt-0.5">
                โค้ดได้รับการจัดระเบียบให้พร้อมนำขึ้น GitHub และเชื่อมต่อฐานข้อมูล Cloud Firestore / RTDB ได้อย่างปลอดภัย
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 rounded-full font-bold">
              ✓ Ready for Git & Firebase
            </span>
          </div>
        </div>
          ...
          <div className="bg-slate-950/60 rounded-2xl p-5 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-indigo-300 font-bold">
                ...
                <span>1. คำสั่ง Push ขึ้น GitHub</span>
              </div>
              <button
                onClick={() => copyToClipboard(`git init
git add .
git commit -m "feat: Apartment Billing with Firebase Sync"
git branch -M main
git remote add origin https://github.com/<USERNAME>/<REPO>.git
git push -u origin main`, 'คำสั่ง Git')}
                className="inline-flex items-center gap-1 px-2 py-1 bg-indigo-600/30 hover:bg-indigo-600/50 border border-indigo-500/30 rounded text-[11px] text-indigo-200 transition cursor-pointer"
              >
                <Copy className="w-3 h-3" />
                <span>คัดลอกคำสั่ง</span>
              </button>
            </div>
            <pre className="font-mono text-[11px] text-slate-300 bg-slate-900/90 p-3 rounded-xl overflow-x-auto leading-relaxed border border-slate-800">
{`git init
git add .
git commit -m "feat: Property Management System"
git branch -M main
git remote add origin https://github.com/<YOUR_USER>/<REPO>.git
git push -u origin main`}
            </pre>
            <p className="text-[11px] text-slate-400">
              * ไฟล์ <code className="text-indigo-300">.gitignore</code> ถูกตั้งค่าเพื่อปกป้องความลับและไฟล์แคชไม่ให้หลุดขึ้นสาธารณะแล้ว
            </p>
          </div>
          ...
          <div className="bg-slate-950/60 rounded-2xl p-5 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-amber-300 font-bold">
                ...
                <span>2. ค่าคอนฟิก Firebase (.env)</span>
              </div>
              <button
...
VITE_FIREBASE_MEASUREMENT_ID="${firebaseConfig.measurementId}"`, 'ไฟล์ .env')}
                className="inline-flex items-center gap-1 px-2 py-1 bg-amber-600/30 hover:bg-amber-600/50 border border-amber-500/30 rounded text-[11px] text-amber-200 transition cursor-pointer"
              >
                <Copy className="w-3 h-3" />
...
VITE_FIREBASE_APP_ID="${firebaseConfig.appId}"`}
            ...
            <p className="text-[11px] text-slate-400">
              * รองรับทั้งการใช้โปรเจกต์เดิม (<code className="text-amber-300">{firebaseConfig.projectId}</code>) หรือโปรเจกต์ใหม่ของคุณเองผ่าน <code className="text-indigo-300">.env</code>
            </p>
          </div>
        </div>

        {/* Step 3: Firebase Hosting & Rules */}
        <div className="pt-4 border-t border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-300">
          <div className="flex items-center gap-2">
            <Flame className="w-4 h-4 text-amber-400" />
            <span>คำสั่ง Deploy เว็บแอปขึ้น Firebase Hosting: <code className="text-amber-300 font-mono bg-slate-900 px-2 py-0.5 rounded border border-slate-700">npm run deploy:firebase</code></span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-slate-400">กฎความปลอดภัย:</span>
            <span className="font-mono text-[11px] bg-slate-900 px-2 py-0.5 rounded text-emerald-300 border border-slate-700">firestore.rules</span>
            <span className="font-mono text-[11px] bg-slate-900 px-2 py-0.5 rounded text-cyan-300 border border-slate-700">database.rules.json</span>
          </div>
        </div>
      </div>
    </div>
