...
import { BuildingProfile, RoomRecord, OccupancyStatus, LandlordConfig } from '../types';
import { getPaymentAccountForBuilding, POPULAR_THAI_BANKS } from '../utils/paymentAccount';
...
export const BuildingManagementView: React.FC<BuildingManagementViewProps> = ({
  ...
  const handleOpenEditModal = (bld: BuildingProfile) => {
    ...
    const resolvedDefaultOption = bld.paymentAccountOption || (bld.name.includes('บ้าน') ? 'house' : 'default');
    setFormData({
      id: bld.id,
      name: bld.name,
      ...
      paymentAccountOption: resolvedDefaultOption,
      customBankName: bld.customBankName || '',
      customBankAccount: bld.customBankAccount || '',
      customPromptPayId: bld.customPromptPayId || '',
      customAccountName: bld.customAccountName || '',
    });
    setFormErrors({});
  };
  ...
  const handleSubmitBuilding = (e: React.FormEvent) => {
    ...
    if (formData.paymentAccountOption === 'custom') {
      if (!formData.customBankName.trim()) {
        errors.customBankName = 'กรุณาระบุชื่อธนาคาร';
      }
      if (!formData.customBankAccount.trim()) {
        errors.customBankAccount = 'กรุณาระบุเลขที่บัญชี';
      }
    }
    ...
    if (editingBuilding) {
      const updated: BuildingProfile = {
        ...
        paymentAccountOption: formData.paymentAccountOption,
        customBankName: formData.paymentAccountOption === 'custom' ? formData.customBankName.trim() : undefined,
        customBankAccount: formData.paymentAccountOption === 'custom' ? formData.customBankAccount.trim() : undefined,
        customPromptPayId: formData.paymentAccountOption === 'custom' ? formData.customPromptPayId.trim() : undefined,
        customAccountName: formData.paymentAccountOption === 'custom' ? formData.customAccountName.trim() : undefined,
      };
    ...
    <div className="space-y-6">
          ...
          return (
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                {(() => {
                  const accountInfo = getPaymentAccountForBuilding(bld, config);
                  return (
                    <div className={`p-2.5 rounded-lg border text-xs flex items-center justify-between ${
                      accountInfo.isHouseAccount 
                        ? 'bg-purple-50/80 border-purple-200 text-purple-950' 
                        : accountInfo.accountType === 'custom'
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-bold flex items-center gap-1.5">
                            <span>{accountInfo.isHouseAccount ? '🏠' : accountInfo.accountType === 'custom' ? '⚙️' : '👑'}</span>
                            <span>{accountInfo.accountLabel}</span>
                          </div>
                          <div className="text-[11px] opacity-80">
                            {accountInfo.bankName} • <span className="font-mono font-semibold">{accountInfo.bankAccount}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/90 border border-slate-200 block">
                          {accountInfo.promptPayId || 'พร้อมเพย์'}
                        </span>
      ...
      {(isAddModalOpen || editingBuilding) && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl max-w-xl w-full my-auto overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
            {/* Modal Header */}
            ...
            {/* Modal Form */}
            <form onSubmit={handleSubmitBuilding} className="flex flex-col min-h-0 flex-1 overflow-hidden">
              ...
              {/* Payment Receiving Account Selector for this Building */}
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-slate-800 flex items-center gap-1.5">
                    <CreditCard className="w-4 h-4 text-blue-600" />
                    <span>บัญชีรับเงินค่าเช่าของอาคารนี้ (Payment Receiving Account)</span>
                  </label>
                  <span className="text-[11px] text-slate-500">
                    เลือกบัญชีที่ต้องการให้แสดงในใบแจ้งหนี้
                  </span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                  {/* Option 1: Default Landlord Account */}
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, paymentAccountOption: 'default' }))}
                    className={`p-3 rounded-xl border text-left transition cursor-pointer flex flex-col justify-between ${
                      formData.paymentAccountOption === 'default'
                  ...
                  >
                    <div>
                      <div className="font-bold text-xs flex items-center gap-1.5">
                        ...
                        {formData.paymentAccountOption === 'default' && <Check className="w-3.5 h-3.5 text-blue-600 ml-auto" />}
                      </div>
                      <p className="text-[11px] text-slate-500 mt-1 truncate">
                        {config?.bankName ? config.bankName.replace(/\s*\([^)]*\)/g, '').trim() : 'ธนาคารกสิกรไทย'}
                      </p>
                      <p className="font-mono text-[11px] font-bold text-slate-800">
                        {config?.bankAccount ? config.bankAccount.replace(/\s*\([^)]*\)/g, '').trim() : '743-2-89012-3'}
                      </p>
                    </div>
                    <span className="text-[10px] text-slate-400 mt-2 block border-t border-slate-200/60 pt-1">
                      {config?.landlordName || 'ผู้จัดการ'}
                    </span>
                  </button>

                  {/* Option 2: House Rental Account */}
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, paymentAccountOption: 'house' }))}
                    className={`p-3 rounded-xl border text-left transition cursor-pointer flex flex-col justify-between ${
                      formData.paymentAccountOption === 'house'
                  ...
                  >
                    <div>
                      <div className="font-bold text-xs flex items-center gap-1.5">
                        <span>🏠 บัญชีบ้านเช่า</span>
                        {formData.paymentAccountOption === 'house' && <Check className="w-3.5 h-3.5 text-purple-600 ml-auto" />}
                      </div>
                      <p className="text-[11px] text-purple-700 mt-1 truncate">
                        {config?.houseBankName ? config.houseBankName.replace(/\s*\([^)]*\)/g, '').trim() : 'ธนาคารไทยพาณิชย์ (SCB)'}
                      </p>
                      <p className="font-mono text-[11px] font-bold text-purple-950">
                        {config?.houseBankAccount ? config.houseBankAccount.replace(/\s*\([^)]*\)/g, '').trim() : '408-2-88910-1'}
                      </p>
                    </div>
                    <span className="text-[10px] text-purple-600 mt-2 block border-t border-purple-200/60 pt-1 font-medium">
                      {config?.houseAccountName || 'คุณพลอย (Ploy)'}
                    </span>
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, paymentAccountOption: 'custom' }))}
                    className={`p-3 rounded-xl border text-left transition cursor-pointer flex flex-col justify-between ${
                      formData.paymentAccountOption === 'custom'
                  ...
                  >
                    <div>
                      <div className="font-bold text-xs flex items-center gap-1.5">
                        <span>⚙️ บัญชีเฉพาะอาคาร</span>
                        {formData.paymentAccountOption === 'custom' && <Check className="w-3.5 h-3.5 text-amber-600 ml-auto" />}
                      </div>
                      <p className="text-[11px] text-slate-500 mt-1 truncate">
                        {formData.customBankName || 'ระบุเอง'}
                      </p>
                      <p className="font-mono text-[11px] font-bold text-slate-800 truncate">
                        {formData.customBankAccount || 'ยังไม่กำหนด'}
                      </p>
                    </div>
                    <span className="text-[10px] text-slate-400 mt-2 block border-t border-slate-200/60 pt-1">
                      กำหนดธนาคารเอง
                ...
                {formData.paymentAccountOption === 'custom' && (
                  <div className="p-3 bg-white rounded-lg border border-amber-300 space-y-3 animate-in fade-in">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        {formErrors.customBankName && (
                          <p className="text-[11px] text-red-600 mt-0.5">{formErrors.customBankName}</p>
                        )}
                        <div className="flex flex-wrap gap-1 mt-1">
                          {POPULAR_THAI_BANKS.slice(0, 5).map((b) => (
                            <button
                              type="button"
                              key={`cust-bld-bank-${b.code}`}
                              onClick={() => setFormData(prev => ({ ...prev, customBankName: b.name }))}
                              className="px-1.5 py-0.5 text-[10px] bg-slate-100 hover:bg-slate-200 rounded text-slate-600"
                            >
                              {b.shortName}
                            </button>
                          ))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormData(prev => ({ ...prev, customBankAccount: e.target.value }))}
                          className="w-full text-xs p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono"
                        />
                        {formErrors.customBankAccount && (
                          <p className="text-[11px] text-red-600 mt-0.5">{formErrors.customBankAccount}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          value={formData.customPromptPayId}
                          onChange={(e) => setFormData(prev => ({ ...prev, customPromptPayId: e.target.value }))}
                          className="w-full text-xs p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormData(prev => ({ ...prev, customAccountName: e.target.value }))}
                          className="w-full text-xs p-2 bg-slate-50 border border-slate-300 rounded-lg"
                        />
                      </div>
                    </div>
              ...
              <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex justify-end gap-2 shrink-0">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition shadow-sm cursor-pointer"
                >
      ...
      {batchModalBuilding && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full my-auto overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 bg-slate-900 text-white flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2">
