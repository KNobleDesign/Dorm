...
import { RoomRecord, LandlordConfig, WaterCalcType, OccupancyStatus, AppUser, BuildingProfile } from '../types';
...
export const InvoiceView: React.FC<InvoiceViewProps> = ({
  ...
  const filteredRooms = rooms.filter(r => {
    if (selectedBuildingFilter !== 'ALL') {
      const match = r.building === selectedBuildingFilter ||
        (r.building && selectedBuildingFilter && (
          r.building.replace(/^(บ้านเช่า|อาคาร)\s*/i, '').trim() === selectedBuildingFilter.replace(/^(บ้านเช่า|อาคาร)\s*/i, '').trim()
        )) ||
        (r.buildingId && buildingProfiles?.find(bp => bp.id === r.buildingId)?.name === selectedBuildingFilter);
  ...
  const handleDownloadFourPerPagePdf = async (targetPage?: number | React.MouseEvent) => {
    ...
    try {
      const pageElements = Array.from(
        pdfMasterContainerRef.current.querySelectorAll('.pdf-a4-page')
      ) as HTMLElement[];
      ...
      await exportElementToA4Pdf(targetEl, fileName, 'portrait', { width: 794, height: 1123, scale: 2 });
      ...
      setPdfErrorMessage('เกิดข้อผิดพลาดในการสร้างไฟล์ PDF 4 ห้องต่อหน้า กรุณาลองใหม่อีกครั้ง');
    } finally {
  ...
  const handleDownloadAllPagesPdf = async () => {
    ...
    try {
      ...
      await exportElementsToMultiPageA4Pdf(
        ...
        fileName,
        (current, total) => {
          setPdfProgress(`กำลังสร้าง PDF 4 ห้องต่อหน้า... (แผ่นที่ ${current}/${total})`);
        },
        'portrait',
      ...
      setPdfErrorMessage('เกิดข้อผิดพลาดในการสร้างไฟล์ PDF ครบทุกหน้า กรุณาลองใหม่อีกครั้ง');
    } finally {
      setIsGeneratingPdf(false);
      setPdfProgress(null);
    }
  ...
  const handleDownloadSinglePdf = async (room: RoomRecord) => {
    ...
    try {
      ...
      const fileName = `ใบแจ้งหนี้_${room.building}_ห้อง_${room.roomNo}_${activeMonth}.pdf`;
      await exportElementToA4Pdf(element, fileName, 'portrait', { width: 794, scale: 2 });
      setPdfSuccessMessage(`ดาวน์โหลดใบแจ้งหนี้ห้อง ${room.roomNo} สำเร็จ!`);
    } catch (err) {
      console.error('Single PDF generation error:', err);
      setPdfErrorMessage('เกิดข้อผิดพลาดในการสร้างไฟล์ PDF กรุณาลองใหม่อีกครั้ง');
    } finally {
      setIsGeneratingPdf(false);
      setPdfProgress(null);
    }
  ...
  const handleExportExcel = () => {
    ...
    const rows = rooms.map(r => {
      ...
      const acc = getPaymentAccountForRoom(r, config, buildingProfiles);
    ...
    <div className="space-y-6">
      ...
      {pdfSuccessMessage && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs flex items-center justify-between animate-in fade-in print:hidden">
          ...
          <button onClick={() => setPdfSuccessMessage(null)} className="text-emerald-600 hover:text-emerald-800 cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {pdfErrorMessage && (
        <div className="p-3 bg-red-50 border border-red-200 text-red-800 rounded-lg text-xs flex items-center justify-between animate-in fade-in print:hidden">
          <div className="flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-red-600 flex-shrink-0" />
            <span>{pdfErrorMessage}</span>
          </div>
          <button onClick={() => setPdfErrorMessage(null)} className="text-red-600 hover:text-red-800 cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-slate-500 flex items-center gap-1.5 flex-wrap">
                            ...
                            <span>•</span>
                            <span>รวมสุทธิ: <strong className="text-slate-800">฿{rGrandTotal.toLocaleString()}</strong></span>
                            {(() => {
                              const acc = getPaymentAccountForRoom(r, config, buildingProfiles);
                              return (
                                <span className={`text-[9.5px] px-1.5 py-0.2 rounded font-medium whitespace-nowrap flex-shrink-0 ${
                                  acc.isHouseAccount
              ...
              <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm print:hidden">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {(() => {
                      const acc = getPaymentAccountForRoom(activePreviewRoom, config, buildingProfiles);
                      return (
                        <span className={`text-[10.5px] px-2 py-0.5 rounded-full border font-medium flex items-center gap-1 whitespace-nowrap ${
                          acc.isHouseAccount
                            ? 'bg-purple-50 border-purple-200 text-purple-800'
                            : 'bg-blue-50 border-blue-200 text-blue-800'
                        }`}>
                          <span>{acc.isHouseAccount ? '🏠' : '👑'}</span>
                          <span>{acc.accountLabel} ({acc.bankName.replace(/\s*\([^)]*\)/g, '').trim()} {acc.bankAccount})</span>
                        </span>
                      );
                    })()}
                    <button
                ...
                >
                  ...
                  {(() => {
                    ...
                    const lFee = activePreviewRoom.lateFeeTotal || ((activePreviewRoom.lateDays || 0) * (activePreviewRoom.lateFeePerDay || config.lateFeePerDayDefault || 100));
                    const grandTot = activePreviewRoom.grandTotal || (activePreviewRoom.total + prevBal + lFee);
                    const paymentAccount = getPaymentAccountForRoom(activePreviewRoom, config, buildingProfiles);
                    ...
                    return (
                      <div className={`border rounded-lg p-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs ${
                        paymentAccount.isHouseAccount
                          ? 'border-purple-200 bg-purple-50/60'
                          : 'border-slate-200 bg-slate-50'
                      }`}>
                        <div className="space-y-1.5">
                          <div className="font-bold text-slate-900 flex items-center gap-2">
                            <CreditCard className={`w-4 h-4 ${paymentAccount.isHouseAccount ? 'text-purple-600' : 'text-blue-600'}`} />
                          ...
                          <div className="text-slate-600 space-y-0.5">
                            <div>• ธนาคาร: <strong>{displayBankName}</strong></div>
                            <div>• เลขที่บัญชี: <strong className="font-mono text-slate-900">{displayBankAccount}</strong></div>
                            <div>• บัญชีพร้อมเพย์: <strong className="font-mono text-slate-900">{paymentAccount.promptPayId || '-'}</strong></div>
                            <div>• ชื่อบัญชี: <strong>{paymentAccount.accountName || '-'}</strong></div>
                          </div>
                        ...
                        {paymentAccount.promptPayId && (
                          <div className="flex flex-col items-center bg-white p-2.5 rounded-lg border border-slate-200 shadow-2xs">
                            <PromptPayQRCode
                              promptPayId={paymentAccount.promptPayId}
                              amount={grandTot}
                              size={88}
                            />
      ...
      {editingRoom && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in print:hidden font-google-sans overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full my-auto p-6 space-y-4 max-h-[90vh] overflow-y-auto">
      ...
      {quickArrearsRoom && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-md w-full my-auto p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
      ...
      {isArrearsManagerOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-4xl w-full my-auto p-6 shadow-2xl border border-slate-200 space-y-4 max-h-[90vh] flex flex-col">
