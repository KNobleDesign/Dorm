...
export const InvoiceView: React.FC<InvoiceViewProps> = ({
  ...
  // 4 Rooms per page PDF generation (Target Sheet or Current Page) - Device Agnostic
  const handleDownloadFourPerPagePdf = async (pageIdx: number = currentPage) => {
    ...
    try {
      ...
      // Small delay to allow browser to complete any layout styling
      ...
      pdf.save(`ใบแจ้งหนี้_4ห้องต่อแผ่น_แผ่นที่_${pageIdx + 1}_งวด_${activeMonth}.pdf`);
  ...
  // Download All 4-Per-Page sheets as a single multi-page PDF - Device Agnostic
  ...
  // Download 4-rooms-per-page sheet containing a specific room
  ...
  // Legacy single slip PDF generator (if user specifically wants full page single slip)
  const handleDownloadSinglePdf = async (room: RoomRecord) => {
    if (!singleInvoiceRef.current) return;
    setIsGeneratingPdf(true);
    setPdfProgress(`กำลังสร้างใบเดี่ยวห้อง ${room.roomNo}...`);
    setPdfSuccessMessage(null);
    ...
    <div className="space-y-6">
      ...
      {viewMode === 'four-per-page' && (
        <div className="space-y-4">
          ...
          {/* Dedicated Off-Screen Master Container for 100% Device-Agnostic PDF Generation */}
          <div
