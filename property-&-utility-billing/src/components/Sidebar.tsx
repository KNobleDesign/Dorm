...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const isPloy = currentUser.role === 'ploy';
  const isOwner = currentUser.role === 'owner' || currentUser.role === 'ploy';
  const isStaff = currentUser.role === 'caretaker' || currentUser.role === 'user';

  const allNavItems = [
    {
      id: 'dashboard' as ActiveTab,
      label: isStaff ? 'Dashboard (สถานะหอพัก)' : 'Dashboard (ภาพรวมรายรับ)',
      seniorLabel: isStaff ? '📊 สรุปสถานะการเข้าพัก & มิเตอร์' : '📊 ภาพรวมและสรุปยอดเงิน',
      shortLabel: 'ภาพรวม',
    ...
    },
    {
      ...
      label: isStaff ? 'Invoices (ใบแจ้งหนี้ - พนักงาน)' : 'Invoices (ใบแจ้งหนี้ 4/แผ่น)',
      seniorLabel: isStaff ? '📄 ใบแจ้งหนี้ (ซ่อนข้อมูลเงิน)' : '📄 พิมพ์ใบแจ้งหนี้ค่าเช่า',
      shortLabel: 'ใบแจ้งหนี้',
      icon: FileText,
      iconColor: isSeniorMode ? 'text-amber-300' : 'text-amber-400',
      badge: isStaff ? 'พนักงาน' : `${rooms.length} ใบ`,
      badgeColor: isStaff ? 'bg-blue-900/60 text-blue-300' : (isSeniorMode ? 'bg-amber-400 text-black font-black' : 'bg-slate-800 text-slate-300'),
    },
    {
      id: 'caretaker-sheet' as ActiveTab,
      label: 'สรุปเก็บเงินผู้ดูแล (1 แผ่น A4)',
      seniorLabel: '📋 ใบสรุปเก็บเงิน 1 แผ่น (ผู้ดูแล)',
      shortLabel: 'สรุป A4',
      icon: ClipboardList,
      iconColor: isSeniorMode ? 'text-amber-300' : 'text-amber-400',
      badge: '1 แผ่น A4',
      badgeColor: isSeniorMode ? 'bg-amber-400 text-black font-black' : 'bg-amber-500/20 text-amber-300 border border-amber-500/30',
    },
    {
      id: 'lease-agreement' as ActiveTab,
      label: 'ทำสัญญาห้องพัก (PDF / Excel)',
      seniorLabel: '📝 ทำสัญญาเช่าห้องพัก (A4/Excel)',
      shortLabel: 'สัญญาเช่า',
      icon: ScrollText,
      iconColor: isSeniorMode ? 'text-blue-300' : 'text-blue-400',
      badge: 'PDF/Excel',
      badgeColor: isSeniorMode ? 'bg-blue-400 text-black font-black' : 'bg-blue-500/20 text-blue-300 border border-blue-500/30',
    },
    {
    ...
    {
      id: 'firebase-sync' as ActiveTab,
      label: 'Firebase & Vercel Deploy 🔥',
      seniorLabel: '🔥 ซิงค์ Firebase & Vercel',
      shortLabel: 'Firebase',
      icon: Flame,
      iconColor: isSeniorMode ? 'text-amber-300' : 'text-amber-400',
      badge: 'Cloud',
      badgeColor: isSeniorMode ? 'bg-amber-400 text-black font-black' : 'bg-amber-500/20 text-amber-300 font-bold',
    },
    {
    ...
    },
    {
      ...
      label: isPloy ? 'จัดการผู้ใช้ (User Admin 💎)' : 'จัดการผู้ใช้ (User Admin 👑)',
      seniorLabel: isPloy ? '💎 จัดการบัญชีผู้ใช้ & สิทธิ์' : '👑 จัดการบัญชีผู้ใช้ & สิทธิ์',
      shortLabel: 'ผู้ใช้งาน',
      icon: Users,
      iconColor: isSeniorMode ? 'text-amber-300' : 'text-amber-400',
      badge: isPloy ? 'Admin' : 'Owner',
      badgeColor: isPloy ? 'bg-purple-400 text-black font-black' : (isSeniorMode ? 'bg-amber-400 text-black font-black' : 'bg-amber-500/20 text-amber-300'),
    },
  ];

  // RBAC Tab Visibility Rules:
  // - Ploy: view and edit everything (all tabs)
  // - Owner (คุณแม่): dashboard, meter-entry, rooms, buildings, invoices, caretaker-sheet, lease-agreement, user-admin
  // - Staff (Caretaker): dashboard, meter-entry, rooms, invoices, caretaker-sheet, lease-agreement
  const allowedTabs: ActiveTab[] = isPloy
    ? ['dashboard', 'meter-entry', 'rooms', 'buildings', 'invoices', 'caretaker-sheet', 'lease-agreement', 'sheet-view', 'schema', 'ios-app', 'firebase-sync', 'gas-code', 'user-admin']
    : isOwner
      ? ['dashboard', 'meter-entry', 'rooms', 'buildings', 'invoices', 'caretaker-sheet', 'lease-agreement', 'user-admin']
      : ['dashboard', 'meter-entry', 'rooms', 'invoices', 'caretaker-sheet', 'lease-agreement'];
  ...
  return (
    ...
    >
        ...
        {!isCollapsed && (
          <div className={`px-3 pt-2.5 pb-2 border-b flex-shrink-0 ${
            ...
            {!isUserSectionCollapsed && (
              <div 
                onClick={isOwner ? onOpenUserManager : undefined}
                ...
                title={isOwner ? "คลิกเพื่อสลับผู้ใช้งาน หรือจัดการบัญชี" : "สิทธิ์พนักงานดูแล (ห้ามสลับเป็นเจ้าของ)"}
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className={`font-bold flex items-center gap-1 ${
                      isSeniorMode 
                        ? (isOwner ? 'text-amber-300 text-xs' : 'text-cyan-300 text-xs') 
                        : (isOwner ? 'text-amber-300 text-[10px]' : 'text-cyan-300 text-[10px]')
                    }`}>
                      {currentUser.role === 'ploy' ? (
                        <>
                          <Crown className="w-3 h-3 text-purple-300" /> เจ้าของ & ดูแลระบบ (ดูได้ทุกอย่าง รวมเงิน)
                        </>
                      ) : currentUser.role === 'owner' ? (
                        <>
                          <Crown className="w-3 h-3" /> เจ้าของ (สิทธิ์เต็ม+การเงิน)
                        </>
                      ) : (
                        <>
        ...
        {!isCollapsed && (
          <div className={`p-3 mx-3 my-2 rounded-xl border flex-shrink-0 transition-all ${
            ...
            >
              <div
                onClick={() => {
                  if (allowedTabs.includes('buildings')) {
                    handleNavClick('buildings');
                  } else {
                    toggleBuildingsCollapsed();
                  }
                }}
                className={`flex items-center gap-2 font-bold transition cursor-pointer ${
                  isSeniorMode ? 'text-xs text-amber-300 hover:text-amber-200' : 'text-[11px] text-slate-300 hover:text-white'
                }`}
                title={allowedTabs.includes('buildings') ? "คลิกเพื่อไปที่หน้าจัดการอาคาร" : "อาคารในระบบ"}
              >
                <Building2 className={isSeniorMode ? "w-4 h-4 text-amber-400" : "w-3.5 h-3.5 text-blue-400"} />
                <span>อาคารในระบบ ({buildings.length})</span>
                {allowedTabs.includes('buildings') && (
                  <span className="text-[9px] px-1 py-0.2 bg-blue-500/20 text-blue-300 rounded font-normal hover:bg-blue-500/30">
                    จัดการ
                  </span>
                )}
              </div>
              <button
                ...
                className="p-1 text-slate-400 group-hover:text-white hover:bg-slate-700 rounded transition cursor-pointer"
                aria-label={isBuildingsCollapsed ? "ขยายรายชื่ออาคาร" : "ย่อรายชื่ออาคาร"}
                title={isBuildingsCollapsed ? "ขยายดูรายชื่ออาคาร" : "ย่อรายชื่ออาคาร"}
              >
              ...
              <div className="mt-2.5 pt-2 border-t border-slate-700/60 space-y-1.5 max-h-32 overflow-y-auto pr-1">
                {buildings.map((b) => {
                  ...
                  return (
                    <div 
                      key={b.id} 
                      onClick={() => allowedTabs.includes('buildings') && handleNavClick('buildings')}
                      className={`flex justify-between items-center text-slate-200 text-xs hover:text-white transition ${
                        allowedTabs.includes('buildings') ? 'cursor-pointer hover:bg-slate-700/40 px-1 py-0.5 rounded' : ''
                      }`}
                      title={allowedTabs.includes('buildings') ? `คลิกเพื่อจัดการอาคาร ${b.name}` : b.name}
                    >
      ...
      <div className={`border-t flex-shrink-0 ${
        ...
        {!isCollapsed && onOpenClearDataModal && (isPloy || isOwner) && (
          <button
            type="button"
            onClick={onOpenClearDataModal}
            className={`w-full py-1.5 px-2.5 rounded-lg flex items-center justify-between text-xs font-bold transition cursor-pointer ${
              isSeniorMode
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span>ล้าง & เริ่มต้นตึก/ห้องใหม่</span>
            </span>
            <span className="text-[10px] px-1.5 py-0.2 bg-white/10 rounded font-mono">Fresh</span>
          </button>
        )}
        ...
        {!isCollapsed ? (
          <div className="flex items-center justify-between gap-2">
            <div 
              onClick={(isPloy || isOwner) ? onOpenUserManager : undefined}
              className={`flex items-center gap-2.5 min-w-0 ${
                (isPloy || isOwner) ? 'cursor-pointer hover:opacity-90 transition' : 'cursor-default'
              }`}
              title={(isPloy || isOwner) ? "จัดการผู้ใช้งาน" : "สิทธิ์พนักงานดูแล (ซ่อนข้อมูลเรื่องเงิน)"}
            >
              <div className={`rounded-full flex items-center justify-center font-bold text-white shadow-xs flex-shrink-0 ${
                ...
                {currentUser.avatar || (isPloy ? '💎' : isOwner ? '👑' : '👷‍♂️')}
              </div>
              <div className="flex flex-col min-w-0">
                ...
                <span className={`truncate ${isSeniorMode ? 'text-[11px] text-amber-300 font-bold' : 'text-[10px] text-slate-400'}`}>
                  {isPloy ? '💎 คุณพลอย (Super Admin)' : isOwner ? '👑 คุณแม่ (เจ้าของหอ)' : '👷‍♂️ พนักงานดูแล (ซ่อนยอดเงิน)'}
                </span>
              </div>
            </div>
            
            <div className="flex items-center gap-1">
              {onOpenClearDataModal && (isPloy || isOwner) && (
                <button
                  type="button"
                  onClick={onOpenClearDataModal}
          ...
          <>
            <button
              type="button"
              onClick={(isPloy || isOwner) ? onOpenUserManager : undefined}
              title={(isPloy || isOwner) ? `จัดการบัญชีผู้ใช้งาน: ${currentUser.name}` : `ผู้ใช้งาน: ${currentUser.name}`}
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-white transition text-base ${
                (isPloy || isOwner) ? 'bg-indigo-900/60 hover:bg-indigo-800 cursor-pointer' : 'bg-slate-800 cursor-default opacity-80'
              }`}
            >
              {currentUser.avatar || (isPloy ? '💎' : isOwner ? '👑' : '👤')}
            </button>
            <button
              type="button"
              onClick={onOpenApartmentSettings}
