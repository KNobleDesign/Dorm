...
export const CaretakerCollectionSheet: React.FC<CaretakerCollectionSheetProps> = ({
  ...
  // Helper to cleanly format building name in the table (shows full building numbers like 399/38, 399/40, ซอย 8, etc.)
  const formatBuildingCleanName = (b?: string): string => {
    if (!b) return '-';
    let cleaned = b.trim();

    // Check specific building number patterns first to ensure they are displayed in full
    if (/399\/38/i.test(cleaned)) {
      return '399/38';
    }
    if (/399\/40/i.test(cleaned)) {
      return '399/40';
    }

    // Strip prefix 'บ้านเช่า' and 'อาคาร'
    cleaned = cleaned
  ...
  // Extract unique building list from rooms and buildingProfiles (deduplicated by clean name)
  const buildingList = useMemo(() => {
    const seen = new Map<string, string>(); // cleanName -> canonicalName
          ...
          {subRooms.map((room, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* 2. ชื่อตึก (แสดงชื่อเต็ม เช่น 399/38, 399/40, ซอย 8 ชัดเจน ไม่ซ่อน ไม่ตัดทอน ไม่บดบัง) */}
                <td
                  style={{
                    padding: `${cellPy} 3px`,
                    fontWeight: 800,
