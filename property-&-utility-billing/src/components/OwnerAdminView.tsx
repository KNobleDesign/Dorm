...
export const OwnerAdminView: React.FC<OwnerAdminViewProps> = ({
  ...
  const [roleFilter, setRoleFilter] = useState<'ALL' | 'ploy' | 'owner' | 'caretaker' | 'user'>('ALL');
  ...
  // Security check: Only owner or ploy role can view this admin dashboard
  if (currentUser.role !== 'owner' && currentUser.role !== 'ploy') {
    return (
      <div className="p-8 max-w-2xl mx-auto text-center space-y-4">
        ...
        <p className="text-sm text-slate-600">
          หน้าการจัดการผู้ใช้งานระบบนี้สงวนสิทธิ์สำหรับบัญชีระดับ <strong>คุณพลอย (Super Admin)</strong> หรือ <strong>เจ้าของหอ (Owner)</strong> เท่านั้น
          บัญชีของคุณ ({currentUser.name}) เป็นระดับพนักงานดูแล ไม่สามารถดูหรือแก้ไขรายชื่อผู้ใช้ได้
        </p>
      </div>
    );
  }
  ...
  const ployCount = users.filter(u => u.role === 'ploy').length;
  const ownerCount = users.filter(u => u.role === 'owner').length;
  const userCount = users.filter(u => u.role === 'caretaker' || u.role === 'user').length;
          ...
          <form onSubmit={handleCreateSubmit} className="p-5 sm:p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  <option value="ploy">💎 เจ้าของ & Super Admin (คุณพลอย - ดูและแก้ไขได้ทุกระบบ รวมถึงการเงิน บันทึกลง Cloud)</option>
                  <option value="owner">👑 Owner / เจ้าของหอ (ดูยอดเงิน แดชบอร์ด จดมิเตอร์ จัดการผู้ใช้)</option>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full text-xs sm:text-sm bg-slate-50 border border-slate-300 rounded-xl p-2.5 font-bold focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    >
                      <option value="ploy">💎 เจ้าของ & Super Admin / คุณพลอย (ดูได้ทุกอย่าง รวมเงิน)</option>
                      <option value="owner">👑 Owner / เจ้าของหอ</option>
                      <option value="caretaker">👷‍♂️ User / พนักงาน (ซ่อนเงิน)</option>
                    </select>
                  </div>
                </div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="inline-flex p-1 bg-slate-100 rounded-xl border border-slate-200 text-xs">
            ...
            <button
              onClick={() => setRoleFilter('ploy')}
              className={`px-3 py-1 rounded-lg font-bold transition cursor-pointer ${
                roleFilter === 'ploy' ? 'bg-white text-purple-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              💎 คุณพลอย ({ployCount})
            </button>
                    ...
                    <td className="py-3.5 px-4">
                      {u.role === 'ploy' ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-900 border border-purple-300">
                          ...
                          <span>เจ้าของ & Super Admin (คุณพลอย - ดูได้ทุกอย่าง รวมเงิน)</span>
                        </span>
                      ) : isOwner ? (
