...
import { CaretakerCollectionSheet } from './CaretakerCollectionSheet';
...
interface DashboardViewProps {
  ...
  onBatchMarkPaid?: (paid: boolean) => void;
  onNavigateToMeter: () => void;
  onNavigateToLease?: () => void;
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [dashboardSubView, setDashboardSubView] = useState<'analytics' | 'caretaker_sheet'>('analytics');
  ...
  const handleExportExcel = () => {
    try {
      ...
      setExportToast('เกิดข้อผิดพลาดในการส่งออก Excel กรุณาลองใหม่อีกครั้ง');
      setTimeout(() => setExportToast(null), 4500);
    }
  };
  ...
  const handleExportCsv = () => {
    try {
      ...
      setExportToast('เกิดข้อผิดพลาดในการส่งออก CSV');
      setTimeout(() => setExportToast(null), 4500);
    }
  };
  ...
  if (dashboardSubView === 'caretaker_sheet') {
    return (
      <div className="space-y-4 font-google-sans">
        {/* Navigation bar to easily return or switch */}
        <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-xs print:hidden">
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs sm:text-sm font-bold transition cursor-pointer shadow-2xs"
            >
              ← กลับหน้าสรุปภาพรวม Dashboard
            </button>
            <span className="text-slate-300 hidden sm:inline">|</span>
            <div className="flex items-center gap-2">
              <span className="text-xs sm:text-sm font-black text-slate-900">
                📋 หน้าสรุปเก็บเงินผู้ดูแลหอ (1 กระดาษ A4 สรุปทุกห้องทุกตึก)
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setDashboardSubView('analytics')}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition cursor-pointer"
            >
              <LayoutDashboard className="w-3.5 h-3.5" />
              <span>ดูสถิติและกราฟรายรับ ➔</span>
            </button>
          </div>
        </div>
        ...
        <CaretakerCollectionSheet
          ...
          config={config}
          buildings={buildings}
          onBackToOverview={() => setDashboardSubView('analytics')}
    ...
    <div className="space-y-6 font-google-sans">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-5 bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5 pt-2 xl:pt-0">
          {/* Caretaker 1 A4 Sheet Button */}
          <button
            onClick={() => setDashboardSubView('caretaker_sheet')}
            className="flex items-center gap-2 px-3.5 sm:px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white text-xs sm:text-sm font-bold rounded-xl transition shadow-md shadow-amber-600/20 cursor-pointer whitespace-nowrap"
            title="เปิดหน้าสรุปเก็บเงิน 1 หน้า A4 (สำหรับปริ้นให้ผู้ดูแลหอเดินเก็บเงิน)"
          >
            <ClipboardList className="w-4 h-4 flex-shrink-0" />
            <span>ใบสรุปเก็บเงินผู้ดูแล (1 แผ่น A4)</span>
          </button>

          {/* Lease Agreement Button */}
          {onNavigateToLease && (
            <button
              onClick={onNavigateToLease}
              className="flex items-center gap-2 px-3.5 sm:px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-bold rounded-xl transition shadow-md shadow-blue-600/20 cursor-pointer whitespace-nowrap"
              title="ทำสัญญาเช่าห้องพัก พร้อมพิมพ์หรือส่งออกเป็น PDF/Excel 4 หน้า A4"
            >
              <ScrollText className="w-4 h-4 flex-shrink-0" />
              <span>ทำสัญญาห้องพัก (A4/Excel)</span>
            </button>
          )}
      ...
      {/* Sub-view Switcher between Analytics Dashboard & 1-A4 Caretaker Sheet */}
      <div className="flex items-center bg-slate-200/80 p-1 rounded-xl w-fit border border-slate-300/80 shadow-2xs gap-1">
        <button
          onClick={() => setDashboardSubView('analytics')}
          className={`flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition cursor-pointer ${
        ...
        >
          ...
          <span>ภาพรวมและสถิติรายรับ</span>
        </button>

        <button
          onClick={() => setDashboardSubView('caretaker_sheet')}
          className="flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition cursor-pointer text-slate-700 hover:text-slate-900 hover:bg-slate-200/50"
        >
          <ClipboardList className="w-4 h-4 text-amber-600" />
          <span>ใบสรุปเก็บเงินผู้ดูแลหอ (1 แผ่น A4)</span>
          <span className="bg-amber-400 text-amber-950 text-[10px] px-1.5 py-0.2 rounded font-black">
            สรุปทุกตึก
          </span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`bg-white p-5 rounded-2xl border shadow-xs flex flex-col justify-between transition hover:border-slate-300 ${
          !isCaretaker && unpaidRooms.length > 0 ? 'border-amber-200 bg-linear-to-b from-white to-amber-50/15' : 'border-slate-200'
        }`}>
          <div>
            <div className="flex justify-between items-start">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                {isCaretaker ? 'การใช้น้ำรวม (Water Units)' : 'ยอดรอเรียกเก็บงวดนี้ (Current Due)'}
              </span>
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
                isCaretaker ? 'bg-blue-50 text-blue-600' : (unpaidRooms.length > 0 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600')
              }`}>
                {isCaretaker ? <Droplet className="w-4 h-4 text-blue-600" /> : <Clock className="w-4 h-4" />}
              </div>
            </div>
            <div className={`text-2xl sm:text-3xl font-black mt-2 ${isCaretaker ? 'text-blue-600' : (unpaidRooms.length > 0 ? 'text-amber-800' : 'text-emerald-600')}`}>
            ...
            {!isCaretaker && (
              <div className="text-[11px] font-bold text-emerald-700 mt-1 flex items-center gap-1">
                <span>✓ หนี้ค้างเก่าเดิมยกมา: ฿{totalLiability.toLocaleString()}</span>
                {totalLiability === 0 && <span className="text-[10px] text-emerald-600 font-normal">(เริ่มต้นระบบ ไม่มีหนี้ค้าง)</span>}
              </div>
            )}
          </div>
          <div className="text-xs text-slate-600 mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between">
            <span>{isCaretaker ? 'ใช้น้ำประปารวม' : `${unpaidRooms.length} ห้องรอชำระรอบนี้`}</span>
            {!isCaretaker && unpaidRooms.length > 0 ? (
              <button
                onClick={() => setShowUnpaidModal(true)}
                className="text-amber-800 font-bold hover:underline cursor-pointer flex items-center gap-0.5 text-xs"
                title="คลิกเพื่อดูรายชื่อห้องที่รอชำระเงินในงวดนี้"
              >
                <span>ดูห้องรอชำระ ({unpaidRooms.length})</span>
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {buildings.map((b) => {
            ...
            const bPaid = bRooms.filter(r => r.isPaid).reduce((sum, r) => sum + getRoomEffectiveTotal(r), 0);
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-50/50">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            >
              ...
              <option value="UNPAID">⏳ รอชำระงวดนี้ (Pending)</option>
              <option value="LIABILITY">⚠️ มียอดค้างเดิม / ค่าปรับ (Arrears)</option>
              <option value="METER_PENDING">📝 รอกรอกมิเตอร์</option>
            </select>

            {/* Batch Status Buttons for Owner/Admin */}
            {!isCaretaker && onBatchMarkPaid && (
              <div className="flex items-center gap-1.5">
                <button
                  onClick={() => onBatchMarkPaid(true)}
                  className="inline-flex items-center gap-1 px-2.5 py-2 text-xs font-bold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 rounded-xl transition cursor-pointer shadow-2xs whitespace-nowrap"
                  title="ทำเครื่องหมายชำระเงินครบแล้วทุกห้องในงวดนี้"
                >
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />
                  <span className="hidden xl:inline">ชำระครบทั้งหมด</span>
                </button>
                <button
                  onClick={() => onBatchMarkPaid(false)}
                  className="inline-flex items-center gap-1 px-2.5 py-2 text-xs font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-300 rounded-xl transition cursor-pointer shadow-2xs whitespace-nowrap"
                  title="ทำเครื่องหมายรอชำระเงินทุกห้องในงวดนี้"
                >
                  <Clock className="w-3.5 h-3.5 text-slate-500 flex-shrink-0" />
                  <span className="hidden xl:inline">รอชำระทั้งหมด</span>
                </button>
              </div>
            )}
                      ...
                      {!isCaretaker && (
                        <td className="py-3.5 px-4 text-center">
                          <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-bold text-xs transition cursor-pointer shadow-2xs whitespace-nowrap ${
                              r.isPaid
                                ? 'bg-emerald-100 hover:bg-emerald-200 text-emerald-900 border border-emerald-300'
                                : 'bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300'
                            }`}
                            title={r.isPaid ? 'คลิกเพื่อเปลี่ยนเป็นรอชำระ' : 'คลิกเพื่อยืนยันการชำระเงิน'}
                          >
                            {r.isPaid ? (
                              <>
      ...
      {showUnpaidModal && !isCaretaker && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-2xl w-full my-auto shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-base text-white">รายชื่อห้องรอชำระเงิน (งวดประจำเดือน {activeMonth})</h3>
                  <p className="text-xs text-slate-300">รอบบิลปกติประจำงวด (หนี้ค้างชำระเดิมยกมา ฿0 ไม่มีประวัติค้างชำระ)</p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 max-h-[60vh] overflow-y-auto divide-y divide-slate-100">
              {unpaidRooms.length === 0 ? (
                <div className="py-12 text-center text-slate-400">
                  ...
                  <p className="text-xs text-slate-400 mt-1">ไม่มีห้องรอชำระในงวดเดือนนี้</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center justify-between bg-amber-50/80 p-3.5 rounded-xl border border-amber-200 text-xs">
                    <div className="text-amber-900 font-medium">
                      รวมทั้งสิ้น <strong className="font-bold text-amber-950 text-sm">{unpaidRooms.length}</strong> ห้อง
                    </div>
                    <div className="text-right">
                      <span className="text-amber-900 font-bold text-base">฿{totalUnpaidAmount.toLocaleString()}</span>
                      <div className="text-[10px] text-amber-700 font-medium">ยอดรอเรียกเก็บประจำงวดนี้</div>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 text-[11px]">
                        <tr>
                          ...
                          <th className="py-2.5 px-3 font-semibold text-right">ยอดเรียกเก็บงวดนี้</th>
                          ...
                          return (
                            <tr key={r.key} className="hover:bg-slate-50 transition">
                              ...
                              <td className="py-2.5 px-3 text-center">
                                <button
                                  onClick={() => onTogglePaymentStatus(r.key)}
                                  className="px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold text-[11px] border border-emerald-300 transition cursor-pointer"
                                >
                                  ทำเป็นชำระแล้ว
                                </button>
                              </td>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <span className="text-xs text-slate-500">
                💡 ไม่มีหนี้ค้างเดิมยกมา (฿0) หากห้องใดไม่ชำระ ระบบจะส่งยอดไปเป็นค้างจ่ายในเดือนถัดไปอัตโนมัติ
              </span>
              <div className="flex items-center gap-2">
                {onBatchMarkPaid && unpaidRooms.length > 0 && (
                  <button
                    onClick={() => {
                      onBatchMarkPaid(true);
