...
export const ExpenseDashboardSection: React.FC<ExpenseDashboardSectionProps> = ({
  ...
  // Dynamic available months from expenses & active month
  ...
  const handleSaveExpense = (e: React.FormEvent) => {
    ...
    if (!formData.title.trim() || isNaN(numAmount) || numAmount <= 0) {
      setFormError('กรุณากรอกชื่อรายการและระบุจำนวนเงินที่ถูกต้อง (มากกว่า 0)');
      return;
    }
    setFormError(null);
  ...
  const buildingComparisonData = useMemo(() => {
    ...
    return list.map(bldName => {
      const bldRooms = rooms.filter(r => r.building === bldName || r.building.replace(/\s+/g, '') === bldName.replace(/\s+/g, ''));
      const rev = bldRooms.reduce((sum, r) => sum + (r.grandTotal || r.total || 0), 0);
      
      const bldExps = expenses.filter(exp => {
        const matchBld = exp.building === bldName || exp.building.replace(/\s+/g, '') === bldName.replace(/\s+/g, '');
  ...
  const monthlyTrendData = useMemo(() => {
    const monthList = ['07 ก.ค.', '08 ส.ค.', '09 ก.ย.'];
    return monthList.map(m => {
      const mExpenses = expenses.filter(exp => {
        const matchBld = selectedBuilding === 'ALL' || exp.building === selectedBuilding || exp.building.replace(/\s+/g, '') === selectedBuilding.replace(/\s+/g, '');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-100 my-8">
            ...
            <form onSubmit={handleSaveExpense} className="space-y-4 mt-4 text-xs">
              {formError && (
                <div className="p-3 bg-red-50 border border-red-200 text-red-700 rounded-xl flex items-center gap-2 animate-in fade-in">
