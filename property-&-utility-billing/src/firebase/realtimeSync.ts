  ...
  try {
    ...
    try {
      if (!isFirestoreBackingOff()) {
        ...
        // A. Unified Main Bundle Doc (lightweight metadata, exclude full 12-month roomsByMonth to stay well within Firestore's 1MB limit)
    ...
    try {
      ...
      try {
        if (!isFirestoreBackingOff()) {
          ...
          // Keep dormitory/main_data bundle up to date for non-roomsByMonth paths (rooms are in monthly_records to prevent >1MB document size)
          if (path !== 'roomsByMonth') {
...
export async function runFirebaseHealthCheck(): Promise<{
  ...
  const t1 = Date.now();
  try {
    const isAppOk = !!firebaseConfig.projectId && firebaseConfig.projectId.trim().length > 0 && !!firebaseConfig.apiKey;
    results.push({
      step: 1,
      title: `ตรวจสอบการตั้งค่า Firebase App (Project ID: ${firebaseConfig.projectId || 'N/A'})`,
      passed: isAppOk,
      message: isAppOk ? `เชื่อมโยง Project ID: ${firebaseConfig.projectId} ถูกต้องและสมบูรณ์` : 'ยังไม่ได้ระบุ Firebase Project ID หรือ API Key',
