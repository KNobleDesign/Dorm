...
const STORAGE_KEYS = {
  ...
  BUILDINGS: 'propmanage_buildings_v3',
  ROOMS: 'propmanage_rooms_by_month_v3',
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleRemoteBuildings = (remoteBuildings: BuildingProfile[]) => {
      ...
      // If a building name changed (e.g. BLD-RNG01 name changed to "อาคารรังสิตภิรมย์"),
      // update all rooms that belong to this building ID across all months
        ...
        Object.keys(prev).forEach(monthKey => {
          ...
          if (Array.isArray(rooms)) {
            next[monthKey] = rooms.map(r => {
              const matchedBld = remoteBuildings.find(b => b.id === r.buildingId);
    ...
    const handleRemoteRooms = (remoteRoomsByMonth: Record<string, RoomRecord[]>) => {
      ...
      Object.keys(remoteRoomsByMonth).forEach((m) => {
        merged[m] = (remoteRoomsByMonth[m] || []).filter(r => r.key !== '399-38-01-2');
      });
      ...
      Object.keys(INITIAL_ROOMS_DATA).forEach((monthKey) => {
        const defaultRooms = INITIAL_ROOMS_DATA[monthKey] || [];
        const dmDefault = defaultRooms.filter(r => r.buildingId === 'BLD-DM01' || r.building.includes('ดอนเมือง'));
        if (dmDefault.length > 0) {
          const currentRooms = merged[monthKey] || [];
          const dmRooms = currentRooms.filter(r => r.buildingId === 'BLD-DM01' || r.building.includes('ดอนเมือง'));
          if (dmRooms.length < 21) {
            const nonDMRooms = currentRooms.filter(r => r.buildingId !== 'BLD-DM01' && !r.building.includes('ดอนเมือง'));
            merged[monthKey] = [...nonDMRooms, ...dmDefault];
          }
        }
      });
  ...
  const [buildings, setBuildings] = useState<BuildingProfile[]>(() => {
    try {
      ...
      if (saved) {
        ...
        const bMap = new Map<string, BuildingProfile>(parsed.map(b => [b.id, b]));
        DEFAULT_BUILDINGS.forEach(def => {
          if (!bMap.has(def.id)) {
            bMap.set(def.id, def);
          } else {
            const existing = bMap.get(def.id)!;
            bMap.set(def.id, {
              ...existing,
              name: def.name,
              totalUnits: def.totalUnits,
              defaultElecRate: def.defaultElecRate,
              defaultWaterRate: def.defaultWaterRate,
            });
          }
        });
  ...
  const [roomsByMonth, setRoomsByMonth] = useState<Record<string, RoomRecord[]>>(() => {
    try {
      ...
      // Ensure Don Mueang rooms (21 rooms) are included across all months
      Object.keys(INITIAL_ROOMS_DATA).forEach((monthKey) => {
        const defaultRooms = INITIAL_ROOMS_DATA[monthKey] || [];
        const dmDefault = defaultRooms.filter(r => r.buildingId === 'BLD-DM01' || r.building.includes('ดอนเมือง'));
        if (dmDefault.length > 0) {
          const currentRooms = cleanedData[monthKey] || [];
          const dmRooms = currentRooms.filter(r => r.buildingId === 'BLD-DM01' || r.building.includes('ดอนเมือง'));
          if (dmRooms.length < 21) {
            const nonDMRooms = currentRooms.filter(r => r.buildingId !== 'BLD-DM01' && !r.building.includes('ดอนเมือง'));
            cleanedData[monthKey] = [...nonDMRooms, ...dmDefault];
          }
        }
      });
