...
export default function App() {
  ...
  // Persist Users list to local storage (persisting account data)
  useEffect(() => {
    try {
  ...
  const handleClearToFreshSetup = (setup: FreshSetupPayload) => {
    try {
      ...
      const freshBuilding: BuildingProfile = {
        ...
        floors: Math.max(1, Math.ceil(setup.roomCount / 10)),
      ...
      showToast(`ล้างข้อมูลและเริ่มต้น "${setup.propertyName}" เรียบร้อยแล้ว! พร้อมให้กรอกข้อมูลจริง`, 'success');
    } catch (e) {
      console.error(e);
  ...
  const handleClearToEmptySlate = (customConfig?: Partial<LandlordConfig>) => {
    try {
      ...
      showToast('ล้างข้อมูลว่างเปล่า 100% เรียบร้อยแล้ว สามารถเริ่มเพิ่มอาคารและห้องพักได้ทันที', 'info');
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleClearMetersAndTenantsOnly = () => {
    try {
      ...
      showToast('รีเซ็ตเลขมิเตอร์และข้อมูลผู้เช่าทั้งหมดเป็น 0 เรียบร้อยแล้ว พร้อมบันทึกงวดใหม่', 'success');
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleResetAllData = () => {
    try {
      ...
      showToast('คืนค่าข้อมูลตัวอย่าง 3 อาคาร 26 ห้อง เรียบร้อยแล้ว!', 'info');
    } catch (e) {
      console.error(e);
    }
  };
