...
import { CaretakerCollectionSheet } from './components/CaretakerCollectionSheet';
...
import { generateAllSeptemberRooms } from './data/septemberRooms';
import { 
  getMonthInfo, 
  parseMonthKey, 
  formatMonthKey, 
...
const STORAGE_KEYS = {
  ...
  BUILDINGS: 'propmanage_buildings_v4',
  ROOMS: 'propmanage_rooms_by_month_v6',
  ...
  AUTH_USER: 'propmanage_auth_user_v2',
  MANUAL_LOGOUT: 'propmanage_manual_logout_flag',
};

// Helper to build or sync October rooms from September rooms
// Guaranteeing all previous meters match September and zero arrears by default
function syncOctoberRoomsFromSeptember(septRooms: RoomRecord[], existingOctRooms?: RoomRecord[]): RoomRecord[] {
  const septMap = new Map(septRooms.map(r => [r.key, r]));
  const octList = (existingOctRooms && existingOctRooms.length >= 78) ? existingOctRooms : septRooms;

  return octList.map(octRoom => {
    const s = septMap.get(octRoom.key) || octRoom;
    // Check if user has already manually keyed in October's current meter
    const hasOctManualEntry = Boolean(
      octRoom.hasMeterUpdated &&
      octRoom.meterUpdatedDate &&
      !octRoom.meterUpdatedDate.startsWith('2026-09-01') &&
      (octRoom.elecCurr > 0 || (octRoom.waterCalcType === 'meter' && octRoom.waterCurr > 0))
    );

    // Starting meter for October comes from September:
    // If September has a recorded reading (elecCurr > 0), use elecCurr. Otherwise September's baseline (elecPrev)
    const elecPrev = (s.elecCurr && s.elecCurr > 0) ? s.elecCurr : (s.elecPrev || 0);
    const waterPrev = (s.waterCalcType === 'meter')
      ? ((s.waterCurr && s.waterCurr > 0) ? s.waterCurr : (s.waterPrev || 0))
      : 0;

    const elecCurr = hasOctManualEntry ? (octRoom.elecCurr || 0) : 0;
    const elecUnits = hasOctManualEntry ? Math.max(0, elecCurr - elecPrev) : 0;
    const elecRate = s.elecRate || octRoom.elecRate || 8;
    const elecCost = hasOctManualEntry ? (elecUnits * elecRate) : 0;

    const waterCurr = (s.waterCalcType === 'meter' && hasOctManualEntry) ? (octRoom.waterCurr || 0) : 0;
    const waterUnits = (s.waterCalcType === 'meter' && hasOctManualEntry) ? Math.max(0, waterCurr - waterPrev) : 0;
    const waterRate = s.waterRate || octRoom.waterRate || 18;
    const waterCost = s.isOccupied
      ? (s.waterCalcType === 'per_person' ? ((s.occupants || 1) * (s.waterPerPersonRate || 100)) : (hasOctManualEntry ? waterUnits * waterRate : 0))
      : 0;

    const rent = s.isOccupied ? (s.rent || 0) : 0;
    ...
    return {
      ...
      waterCost,
      otherFees: 0,
      previousBalance: 0, // ไม่มีค้างจ่ายเป็น default
      lateDays: 0,
      lateFeeTotal: 0,
      liabilityTotal: 0,
      total,
      grandTotal: total,
      hasMeterUpdated: hasOctManualEntry,
      meterUpdatedDate: hasOctManualEntry ? octRoom.meterUpdatedDate : undefined,
      isPaid: hasOctManualEntry ? octRoom.isPaid : false,
      notes: s.isOccupied
        ? (octRoom.isPaid ? 'ชำระเงินแล้ว' : (hasOctManualEntry ? 'รอชำระเงินตามรอบบิล' : 'รอกดบันทึกมิเตอร์'))
        : 'ห้องว่าง',
    };
  });
}

// Guarantee users list always contains Ploy (Super Admin), Mom (Owner), and Staff
export const normalizeAppUsers = (rawUsers: AppUser[] | null | undefined): AppUser[] => {
  const list: AppUser[] = Array.isArray(rawUsers) && rawUsers.length > 0 ? [...rawUsers] : [...DEFAULT_APP_USERS];

  // 1. Ensure Ploy (Super Admin) is always first and fully populated
  const ployDefault = DEFAULT_APP_USERS[0];
  const ployIdx = list.findIndex(u => u.username === 'ploy' || u.role === 'ploy' || u.id === 'user-ploy');
  if (ployIdx >= 0) {
    list[ployIdx] = {
      ...ployDefault,
      ...list[ployIdx],
      ...
      role: 'ploy',
      username: 'ploy',
      name: list[ployIdx].name || ployDefault.name,
      avatar: '💎',
      phone: list[ployIdx].phone || '084-041-1115',
      password: list[ployIdx].password || '0840411115',
      pinCode: list[ployIdx].pinCode || list[ployIdx].password || '0840411115',
      notes: list[ployIdx].notes || ployDefault.notes,
    };
  } else {
    list.unshift({ ...ployDefault });
  }

  // 2. Ensure Owner (คุณแม่) exists
  const ownerDefault = DEFAULT_APP_USERS[1];
  const ownerIdx = list.findIndex(u => u.role === 'owner' || u.username === 'mom' || u.username === 'admin');
  if (ownerIdx >= 0) {
    list[ownerIdx] = {
      ...ownerDefault,
      ...list[ownerIdx],
      id: list[ownerIdx].id || 'user-owner',
      role: 'owner',
      username: list[ownerIdx].username || 'mom',
      name: list[ownerIdx].name || ownerDefault.name,
      avatar: '👑',
      phone: list[ownerIdx].phone || '081-987-6543',
      password: list[ownerIdx].password || '1234',
      pinCode: list[ownerIdx].pinCode || '1234',
      notes: list[ownerIdx].notes || ownerDefault.notes,
    };
  } else {
    list.push({ ...ownerDefault });
  }

  // 3. Ensure Staff exists
  const staffDefault = DEFAULT_APP_USERS[2];
  const staffIdx = list.findIndex(u => u.role === 'caretaker' || u.username === 'staff');
  if (staffIdx >= 0) {
    list[staffIdx] = {
      ...staffDefault,
      ...list[staffIdx],
      id: list[staffIdx].id || 'user-staff',
      role: 'caretaker',
      avatar: '👷‍♂️',
      password: list[staffIdx].password || '1234',
      pinCode: list[staffIdx].pinCode || '1234',
      notes: list[staffIdx].notes || staffDefault.notes,
    };
  } else {
    list.push({ ...staffDefault });
  }
...
export default function App() {
  ...
  const [activeMonth, setActiveMonth] = useState<string>('09 ก.ย.');
  ...
  const [users, setUsers] = useState<AppUser[]>(() => {
    try {
      ...
      if (saved) {
        ...
        return normalizeAppUsers(parsed);
      }
      return normalizeAppUsers(DEFAULT_APP_USERS);
    } catch {
      return normalizeAppUsers(DEFAULT_APP_USERS);
    }
  });

  // Session persistence: user must log in; active user is kept only during an active browser session
  ...
  const [userManagerTab, setUserManagerTab] = useState<'accounts' | 'my-pin'>('accounts');
  ...
  useEffect(() => {
    ...
    const handleRemoteBuildings = (remoteBuildings: BuildingProfile[]) => {
      isRemoteUpdateRef.current = true;
      const validIds = new Set(DEFAULT_BUILDINGS.map(b => b.id));
      const filtered = remoteBuildings.filter(b => validIds.has(b.id));
      const bMap = new Map<string, BuildingProfile>();
      DEFAULT_BUILDINGS.forEach(def => bMap.set(def.id, def));
      filtered.forEach(b => bMap.set(b.id, b));
      const finalBuildings = Array.from(bMap.values());

      setBuildings(finalBuildings);
      try {
        localStorage.setItem(STORAGE_KEYS.BUILDINGS, JSON.stringify(finalBuildings));
      } catch (e) {
        console.error(e);
      }

      // If a building name changed, update all rooms that belong to this building ID
      setRoomsByMonth(prev => {
        let changed = false;
        const next: Record<string, RoomRecord[]> = {};
        Object.keys(prev).forEach(monthKey => {
          const rooms = prev[monthKey];
          if (Array.isArray(rooms)) {
            next[monthKey] = rooms.map(r => {
              const matchedBld = finalBuildings.find(b => b.id === r.buildingId);
              if (matchedBld && matchedBld.name !== r.building) {
                changed = true;
                return { ...r, building: matchedBld.name, buildingId: matchedBld.id };
              }
              return r;
            });
          } else {
        ...
        if (changed) {
          try {
            localStorage.setItem(STORAGE_KEYS.ROOMS, JSON.stringify(next));
          } catch (e) {
            console.error(e);
          }
          return next;
    ...
    const handleRemoteRooms = (remoteRoomsByMonth: Record<string, RoomRecord[]>) => {
      isRemoteUpdateRef.current = true;
      const validBuildingIds = new Set(DEFAULT_BUILDINGS.map(b => b.id));
      const merged: Record<string, RoomRecord[]> = {};
      Object.keys(remoteRoomsByMonth).forEach((m) => {
        merged[m] = (remoteRoomsByMonth[m] || []).filter(r => r.key !== '399-38-01-2' && validBuildingIds.has(r.buildingId));
      });
      // Ensure September (start month / fresh setup) has zero past arrears/debts and all meters are pending entry ("รอกรอก")
      if (merged['09 ก.ย.']) {
        merged['09 ก.ย.'] = merged['09 ก.ย.'].map(r => {
          const isManuallyEntered = Boolean(r.hasMeterUpdated && r.meterUpdatedDate && !r.meterUpdatedDate.startsWith('2026-09-01 10:00') && (r.elecCurr > r.elecPrev || r.waterCurr > r.waterPrev));
          const elecCurr = isManuallyEntered ? (r.elecCurr || 0) : 0;
          const elecUnits = isManuallyEntered ? (r.elecUnits || 0) : 0;
          const elecCost = isManuallyEntered ? (r.elecCost || 0) : 0;
          const waterCurr = (r.waterCalcType === 'meter' && isManuallyEntered) ? (r.waterCurr || 0) : 0;
          const waterUnits = (r.waterCalcType === 'meter' && isManuallyEntered) ? (r.waterUnits || 0) : 0;
          const waterCost = r.isOccupied
            ? (r.waterCalcType === 'per_person' ? ((r.occupants || 1) * 100) : (isManuallyEntered ? (r.waterCost || 0) : 0))
            : 0;
          const rent = r.isOccupied ? (r.rent || 0) : 0;
          const total = rent + waterCost + elecCost;
          return {
            ...r,
            hasMeterUpdated: isManuallyEntered,
            meterUpdatedDate: isManuallyEntered ? r.meterUpdatedDate : undefined,
            elecCurr,
            elecUnits,
            elecCost,
            waterCurr,
            ...
            grandTotal: total,
            notes: r.isOccupied
              ? (r.isPaid ? 'ชำระเงินแล้ว' : (isManuallyEntered ? 'รอชำระเงินตามรอบบิล' : 'รอกดบันทึกมิเตอร์'))
              : 'ห้องว่าง',
          };
        });
      }
      // Ensure all 78 September rooms from the PDF exist
      const defaultSepRooms = INITIAL_ROOMS_DATA['09 ก.ย.'] || [];
      if (!merged['09 ก.ย.'] || merged['09 ก.ย.'].length < 78) {
        const existingMap = new Map((merged['09 ก.ย.'] || []).map(r => [r.key, r]));
        merged['09 ก.ย.'] = defaultSepRooms.map(def => existingMap.get(def.key) || def);
      }

      // Ensure October (10 ต.ค.) takes previous readings from September with zero arrears as default
      if (merged['09 ก.ย.']) {
        merged['10 ต.ค.'] = syncOctoberRoomsFromSeptember(merged['09 ก.ย.'], merged['10 ต.ค.']);
      }

      setRoomsByMonth(merged);
      try {
        localStorage.setItem(STORAGE_KEYS.ROOMS, JSON.stringify(merged));
      } catch (e) {
        console.error(e);
      }
      setTimeout(() => { isRemoteUpdateRef.current = false; }, 800);
    ...
    const handleRemoteUsers = (remoteUsers: AppUser[]) => {
      isRemoteUpdateRef.current = true;
      const normalized = normalizeAppUsers(remoteUsers);
      setUsers(normalized);
      try {
    ...
    // Immediate initial sync from Firebase Cloud Firestore to ensure fresh data
    pullAllDataFromFirebase().then((res) => {
      if (!res.error) {
        if (res.config) handleRemoteConfig(res.config);
        if (res.buildings && res.buildings.length > 0) handleRemoteBuildings(res.buildings);
        if (res.roomsByMonth && Object.keys(res.roomsByMonth).length > 0) handleRemoteRooms(res.roomsByMonth);
        if (res.expenses && res.expenses.length > 0) handleRemoteExpenses(res.expenses);
        if (res.users && res.users.length > 0) handleRemoteUsers(res.users);
      }
      setTimeout(() => {
  ...
  useEffect(() => {
    try {
      ...
      if (isInitialSyncDoneRef.current && !isRemoteUpdateRef.current) {
  ...
  const handleLogin = (user: AppUser) => {
    ...
    setCurrentUser(user);
    const roleBadge = user.role === 'ploy' ? '💎 คุณพลอย (Owner & Super Admin - ดูได้ทุกอย่าง รวมเงิน)' : user.role === 'owner' ? '👑 เจ้าของหอ' : '👷‍♂️ พนักงาน';
    showToast(`เข้าสู่ระบบสำเร็จ ยินดีต้อนรับ ${user.name} (${roleBadge})`, 'success');
  };
  ...
  const [landlordConfig, setLandlordConfig] = useState<LandlordConfig>(() => {
    try {
      ...
      if (merged.bankAccount) {
        merged.bankAccount = merged.bankAccount.replace(/\s*\([^)]*\)/g, '').trim();
      }
      if (merged.bankName) {
        merged.bankName = merged.bankName.replace(/\s*\([^)]*\)/g, '').trim();
      }
      return merged;
    } catch {
  ...
  const [buildings, setBuildings] = useState<BuildingProfile[]>(() => {
    try {
      ...
      if (saved) {
        const parsed: BuildingProfile[] = JSON.parse(saved);
        const validIds = new Set(DEFAULT_BUILDINGS.map(b => b.id));
        const filtered = parsed.filter(b => validIds.has(b.id));
        const bMap = new Map<string, BuildingProfile>();
        DEFAULT_BUILDINGS.forEach(def => bMap.set(def.id, def));
        filtered.forEach(b => {
          bMap.set(b.id, b);
  ...
  const [roomsByMonth, setRoomsByMonth] = useState<Record<string, RoomRecord[]>>(() => {
    try {
      ...
      // Ensure only rooms belonging to valid buildings are kept and otherFees is 0
      const cleanedData: Record<string, RoomRecord[]> = {};
      Object.keys(rawData).forEach((monthKey) => {
        cleanedData[monthKey] = (rawData[monthKey] || [])
          .filter(room => room.key !== '399-38-01-2' && validBuildingIds.has(room.buildingId))
          .map((room) => {
            ...
            // In September (the initial master data month / fresh setup), strictly no arrears, all meters pending entry ("รอกรอก")
            if (monthKey === '09 ก.ย.') {
              const isManuallyEntered = Boolean(room.hasMeterUpdated && room.meterUpdatedDate && !room.meterUpdatedDate.startsWith('2026-09-01 10:00') && (room.elecCurr > room.elecPrev || room.waterCurr > room.waterPrev));
              const elecCurr = isManuallyEntered ? (room.elecCurr || 0) : 0;
              const elecUnits = isManuallyEntered ? (room.elecUnits || 0) : 0;
              const elecCost = isManuallyEntered ? (room.elecCost || 0) : 0;
              const waterCurr = (room.waterCalcType === 'meter' && isManuallyEntered) ? (room.waterCurr || 0) : 0;
              const waterUnits = (room.waterCalcType === 'meter' && isManuallyEntered) ? (room.waterUnits || 0) : 0;
              const waterCost = room.isOccupied
                ? (room.waterCalcType === 'per_person' ? ((room.occupants || 1) * 100) : (isManuallyEntered ? (room.waterCost || 0) : 0))
                : 0;
              const cleanTotal = (room.isOccupied ? (room.rent || 0) : 0) + waterCost + elecCost;
              return {
                ...room,
                hasMeterUpdated: isManuallyEntered,
                meterUpdatedDate: isManuallyEntered ? room.meterUpdatedDate : undefined,
                elecCurr,
                elecUnits,
                elecCost,
                waterCurr,
                ...
                notes: room.isOccupied
                  ? (room.isPaid ? 'ชำระเงินแล้ว' : (isManuallyEntered ? 'รอชำระเงินตามรอบบิล' : 'รอกดบันทึกมิเตอร์'))
                  : 'ห้องว่าง',
              };
            }
      ...
      // Ensure all 78 September rooms from the PDF exist
      const defaultSepRooms = INITIAL_ROOMS_DATA['09 ก.ย.'] || [];
      if (!cleanedData['09 ก.ย.'] || cleanedData['09 ก.ย.'].length < 78) {
        const existingMap = new Map((cleanedData['09 ก.ย.'] || []).map(r => [r.key, r]));
        cleanedData['09 ก.ย.'] = defaultSepRooms.map(def => existingMap.get(def.key) || def);
      }

      // Ensure October (10 ต.ค.) takes previous readings from September with zero arrears as default
      if (cleanedData['09 ก.ย.']) {
        cleanedData['10 ต.ค.'] = syncOctoberRoomsFromSeptember(cleanedData['09 ก.ย.'], cleanedData['10 ต.ค.']);
      }
  ...
  // 4. Reset All Back to PDF Master Data
  const handleResetAllData = () => {
    try {
      ...
      const freshRooms = generateAllSeptemberRooms();
      setRoomsByMonth(freshRooms);
      setExpenses(DEFAULT_EXPENSES);

      // Instantly sync data to Firebase
      pushAllDataToFirebase({
        config: DEFAULT_LANDLORD_CONFIG,
        buildings: DEFAULT_BUILDINGS,
        roomsByMonth: freshRooms,
        expenses: DEFAULT_EXPENSES,
        users: DEFAULT_APP_USERS,
      }).catch(err => console.warn("Firebase restore note:", err));

      showToast('คืนค่าข้อมูลเริ่มต้นตาม PDF (6 อาคาร 78 ห้อง) เรียบร้อยแล้ว (ซิงค์ Firebase Cloud สำเร็จ)', 'info');
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleResetToPdfData = async () => {
    try {
      ...
      setBuildings(DEFAULT_BUILDINGS);
      setRoomsByMonth(freshRooms);
      localStorage.setItem(STORAGE_KEYS.BUILDINGS, JSON.stringify(DEFAULT_BUILDINGS));
      localStorage.setItem(STORAGE_KEYS.ROOMS, JSON.stringify(freshRooms));
      ...
      showToast('รีเซ็ตข้อมูลอาคารและห้องพักตาม PDF (6 อาคาร 78 ห้อง) เริ่มต้นงวด ก.ย. และส่งขึ้น Firebase สำเร็จแล้ว', 'success');
    } catch (e: any) {
      console.error(e);
      showToast('เกิดข้อผิดพลาดในการรีเซ็ต: ' + (e?.message || 'Unknown error'), 'error');
    }
  };
  ...
  const handleMonthChange = (newMonth: string) => {
    ...
    setRoomsByMonth(prev => {
      // If switching to October ('10 ต.ค.'):
      // Ensure that if October has not had meters entered yet,
      // it dynamically syncs previous meters from September ('09 ก.ย.') with zero arrears by default
      if (newMonth === '10 ต.ค.' && prev['09 ก.ย.']) {
        const syncedOct = syncOctoberRoomsFromSeptember(prev['09 ก.ย.'], prev['10 ต.ค.']);
        return {
          ...prev,
          ['10 ต.ค.']: syncedOct,
        };
      ...
      // Initialize rooms for the new month automatically carrying over unpaid balances if configured
      const newRooms = initializeNewMonthRooms(newMonth, prev, prev['09 ก.ย.'] || prev['08 ส.ค.'] || Object.values(prev)[0] || null, landlordConfig);
      return {
        ...prev,
        [newMonth]: newRooms,
      };
  ...
  const handleSaveMeterReading = (
    ...
    setRoomsByMonth(prev => {
      ...
      const nextRoomsState = {
        ...prev,
        [activeMonth]: monthData,
      };

      if (activeMonth === '09 ก.ย.' && nextRoomsState['10 ต.ค.']) {
        nextRoomsState['10 ต.ค.'] = syncOctoberRoomsFromSeptember(monthData, nextRoomsState['10 ต.ค.']);
      }

      return nextRoomsState;
    });
  ...
  const handleTogglePaymentStatus = (key: string) => {
    setRoomsByMonth(prev => {
      ...
      // Only update next month's arrears (previousBalance) if carryOverArrearsDefault is explicitly true
      const nextMonthKey = getNextMonthKey(activeMonth);
      if (landlordConfig.carryOverArrearsDefault && nextState[nextMonthKey]) {
      ...
      showToast(
        `เปลี่ยนสถานะห้อง ${currentRoom.roomNo} เป็น: ${nextPaid ? '✅ ชำระแล้ว' : '⏳ รอชำระ'}`,
        'info'
      );
  ...
  const handleBatchMarkPaid = (paid: boolean) => {
    setRoomsByMonth(prev => {
      ...
      const updated = monthData.map(room => {
        if (!room.isOccupied) return room;
        return {
          ...room,
          isPaid: paid,
          paymentDate: paid ? (room.paymentDate || todayStr) : undefined,
          notes: paid ? 'ชำระเงินเรียบร้อยแล้ว' : 'รอชำระเงินตามรอบบิล',
        };
      });
      const nextState = { ...prev, [activeMonth]: updated };
      try {
        localStorage.setItem(STORAGE_KEYS.ROOMS, JSON.stringify(nextState));
      } catch (e) {
        console.error(e);
    ...
    showToast(
      paid
        ? `ทำเครื่องหมาย "ชำระครบแล้วทั้งหมด" สำหรับงวด ${activeMonth} เรียบร้อยแล้ว`
        : `ทำเครื่องหมาย "รอชำระเงินทั้งหมด" สำหรับงวด ${activeMonth} เรียบร้อยแล้ว`,
      'success'
    );
  };
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-google-sans text-slate-900 overflow-hidden antialiased">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto bg-slate-50">
        {/* Top Navbar */}
        <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-3 sm:px-5 py-2 flex items-center justify-between shadow-xs top-navbar-single-line gap-2.5 sm:gap-3 flex-nowrap shrink-0 print:hidden">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-shrink-0 flex-nowrap">
            ...
            <h1 className="text-xs sm:text-sm md:text-base font-bold text-slate-800 tracking-tight truncate flex-shrink-0">
              ...
              {activeTab === 'caretaker-sheet' && 'ใบสรุปเก็บเงินผู้ดูแลหอพัก (1 แผ่น A4)'}
              {activeTab === 'lease-agreement' && 'สัญญาเช่าห้องพัก (PDF / Excel A4)'}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 flex-nowrap">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0 flex-nowrap">
              <button
                onClick={() => {
                  if (currentUser.role === 'ploy' || currentUser.role === 'owner') {
                    setUserManagerTab('accounts');
                  } else {
                    setUserManagerTab('my-pin');
                  }
                  setIsUserManagerOpen(true);
                }}
                title={currentUser.role === 'ploy' || currentUser.role === 'owner' ? "จัดการบัญชีผู้ใช้และรหัสผ่าน (User Management)" : "เปลี่ยนรหัสผ่านของฉัน (Change Password)"}
              ...
              >
                <span className="text-sm flex-shrink-0">{currentUser.avatar || (currentUser.role === 'ploy' ? '💎' : currentUser.role === 'owner' ? '👑' : '👷‍♂️')}</span>
                <span className="truncate max-w-[85px] sm:max-w-[120px]">{currentUser.name}</span>
                {currentUser.role === 'ploy' ? (
                  <span className="hidden md:inline-block px-1.5 py-0.2 rounded text-[10px] bg-purple-200/90 text-purple-950 font-black flex-shrink-0">
                    เจ้าของ & Admin 💎
                  </span>
                ) : currentUser.role === 'owner' ? (
                  <span className="hidden md:inline-block px-1.5 py-0.2 rounded text-[10px] bg-amber-200/90 text-amber-950 font-black flex-shrink-0">
                    เจ้าของ 👑
                  </span>
                ) : (
                  <span className="hidden md:inline-block px-1.5 py-0.2 rounded text-[10px] bg-blue-200/90 text-blue-950 font-black flex-shrink-0">
                    พนักงาน 👷‍♂️
                  </span>
                )}
              </button>
              ...
              <button
                ...
                title="ออกจากระบบ (Log Out) เพื่อเข้าสู่ระบบด้วยบัญชีอื่น"
                className="flex items-center gap-1 px-2 sm:px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 hover:text-red-900 border border-red-200 rounded-lg text-xs font-bold transition cursor-pointer flex-shrink-0 shadow-2xs"
              >
                <LogOut className="w-3.5 h-3.5 text-red-600 flex-shrink-0" />
                <span className="whitespace-nowrap hidden sm:inline">ออกจากระบบ</span>
                <span className="whitespace-nowrap sm:hidden">ออก</span>
              </button>
            </div>
        ...
        <div className="p-3 sm:p-6 pb-28 md:pb-8 max-w-7xl w-full mx-auto space-y-4 sm:space-y-6">
          {activeTab === 'dashboard' && (
            <DashboardView
              ...
              onBatchMarkPaid={handleBatchMarkPaid}
              onNavigateToMeter={() => setActiveTab('meter-entry')}
              onNavigateToLease={() => setActiveTab('lease-agreement')}
              onOpenApartmentSettings={() => setIsApartmentModalOpen(true)}
            />
          )}
          ...
          {activeTab === 'buildings' && (
            <BuildingManagementView
              ...
              onNavigateToRooms={(bldFilter) => {
                if (bldFilter) setRoomBuildingFilter(bldFilter);
                setActiveTab('rooms');
              }}
              onOpenClearDataModal={() => setIsClearDataModalOpen(true)}
          ...
          {activeTab === 'rooms' && (
            <RoomBuildingManagerView
              ...
              buildings={buildings}
              config={landlordConfig}
              initialBuildingFilter={roomBuildingFilter}
          ...
          {activeTab === 'caretaker-sheet' && (
            <CaretakerCollectionSheet
              rooms={currentRooms}
              activeMonth={activeMonth}
              buildings={buildingNames}
              buildingProfiles={buildings}
        ...
        <nav aria-label="Mobile Navigation" className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 backdrop-blur-lg border-t border-slate-800 px-2 py-1.5 flex items-center justify-around shadow-2xl h-[56px]">
          <button
