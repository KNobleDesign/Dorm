...
export const RAW_SEPTEMBER_SPECS: RawRoomSpec[] = [
  ...
  { key: '399-40-02', buildingId: 'BLD-399-40', building: 'อาคาร 399/40', roomNo: '2', tenantName: 'คุณ สีบอร์', elecCurr: 491, elecPrev: 420, waterCurr: 62, waterPrev: 50, occupants: 1, waterCalcType: 'meter', rent: 4800, waterRate: 18, elecRate: 8 },
  ...
  { key: '399-40-05', buildingId: 'BLD-399-40', building: 'อาคาร 399/40', roomNo: '5', tenantName: 'คุณ แซน', elecCurr: 765, elecPrev: 700, waterCurr: 28, waterPrev: 20, occupants: 1, waterCalcType: 'meter', rent: 4500, waterRate: 18, elecRate: 8 },
  ...
  { key: 'DM-02', buildingId: 'BLD-DM01', building: 'อาคาร ดอนเมือง', roomNo: '2', tenantName: 'คุณขวัญทิตา', elecCurr: 2328, elecPrev: 2250, waterCurr: 448, waterPrev: 435, occupants: 1, waterCalcType: 'meter', rent: 2800, waterRate: 18, elecRate: 8 },
  ...
  { key: 'DM-05', buildingId: 'BLD-DM01', building: 'อาคาร ดอนเมือง', roomNo: '5', tenantName: 'คุณ ฟริ้ง', elecCurr: 5693, elecPrev: 5600, waterCurr: 411, waterPrev: 395, occupants: 1, waterCalcType: 'meter', rent: 2000, waterRate: 18, elecRate: 8 },
  ...
  { key: 'DM-14', buildingId: 'BLD-DM01', building: 'อาคาร ดอนเมือง', roomNo: '14', tenantName: 'คุณกัณณ์', elecCurr: 7541, elecPrev: 7460, waterCurr: 502, waterPrev: 488, occupants: 1, waterCalcType: 'meter', rent: 2500, waterRate: 18, elecRate: 8 },
  { key: 'DM-15', buildingId: 'BLD-DM01', building: 'อาคาร ดอนเมือง', roomNo: '15', tenantName: 'คุณ ก้ำ', elecCurr: 1130, elecPrev: 1060, waterCurr: 510, waterPrev: 495, occupants: 1, waterCalcType: 'meter', rent: 2500, waterRate: 18, elecRate: 8 },
...
export function buildRoomRecord(spec: RawRoomSpec, monthIndex: number, monthStr: string): RoomRecord {
  // monthIndex: 8 = 09 ก.ย. (September 2569 - Canonical Start Month)
  const isTargetMonth = monthIndex === 8;
  const isSetupOrFuture = monthIndex >= 8;
  ...
  // Baseline readings from PDF
  const baseElec = spec.elecCurr > 0 ? spec.elecCurr : (spec.elecPrev || 0);
  const baseWater = (spec.waterCurr !== undefined && spec.waterCurr > 0) ? spec.waterCurr : (spec.waterPrev || 0);

  let elecPrev = baseElec;
  let elecCurr = 0;
  let elecUnits = 0;
  let elecCost = 0;

  let waterPrev = spec.waterCalcType === 'meter' ? baseWater : 0;
  let waterCurr = 0;
  let waterUnits = 0;
  let waterCost = 0;

  let hasMeterUpdated = false;
  let meterUpdatedDate: string | undefined = undefined;
  let isPaid = false;
  ...
  if (isSetupOrFuture) {
    // In fresh system setup (September onwards), ALL meters are pending entry ("รอกรอก")
    // Previous meter is the starting number from PDF; current reading is 0 (waiting to be entered)
    elecPrev = baseElec;
    elecCurr = 0;
    elecUnits = 0;
    elecCost = 0;
    ...
    if (isOccupied) {
      ...
      waterPrev = spec.waterCalcType === 'meter' ? baseWater : 0;
      waterCurr = 0;
      waterUnits = 0;
      waterCost = 0;
    }
    ...
    // Historical demo months prior to September
    const offset = monthIndex - 8;
    const elecStep = 50;
    const waterStep = 10;
    if (isOccupied) {
      elecCurr = Math.max(0, baseElec + offset * elecStep);
      elecPrev = Math.max(0, baseElec + (offset - 1) * elecStep);
      elecUnits = Math.max(0, elecCurr - elecPrev);
      ...
      if (spec.waterCalcType === 'per_person') {
        ...
        waterCurr = Math.max(0, baseWater + offset * waterStep);
        waterPrev = Math.max(0, baseWater + (offset - 1) * waterStep);
        waterUnits = Math.max(0, waterCurr - waterPrev);
        waterCost = waterUnits * waterRate;
      }
      hasMeterUpdated = true;
      meterUpdatedDate = `2026-${paddedMonth}-01 10:00`;
      isPaid = true;
      paymentDate = `2026-${paddedMonth}-05`;
    }
  }
  ...
  return {
    ...
    meterUpdatedDate,
    notes: isOccupied
      ? (isPaid ? 'ชำระเงินแล้ว' : (hasMeterUpdated ? 'รอชำระเงินตามรอบบิล' : 'รอกดบันทึกมิเตอร์'))
      : 'ห้องว่าง',
  };
}

export function buildSubsequentMonthRoomRecord(sourceRoom: RoomRecord, _targetMonthStr: string): RoomRecord {
  const elecPrev = (sourceRoom.elecCurr && sourceRoom.elecCurr > 0) ? sourceRoom.elecCurr : sourceRoom.elecPrev;
  const waterPrev = (sourceRoom.waterCalcType === 'meter')
    ? ((sourceRoom.waterCurr && sourceRoom.waterCurr > 0) ? sourceRoom.waterCurr : sourceRoom.waterPrev)
    : 0;
  const waterUnits = 0;
  const waterCost = sourceRoom.isOccupied
    ? (sourceRoom.waterCalcType === 'per_person' ? ((sourceRoom.occupants || 1) * (sourceRoom.waterPerPersonRate || 100)) : 0)
    : 0;
  const rent = sourceRoom.isOccupied ? sourceRoom.rent : 0;
  ...
  return {
    ...
    hasMeterUpdated: false,
    meterUpdatedDate: undefined,
    notes: sourceRoom.isOccupied ? 'รอกดบันทึกมิเตอร์' : 'ห้องว่าง',
  };
}
...
export function generateAllSeptemberRooms(): Record<string, RoomRecord[]> {
  ...
  // Months 0-7: Historical demo months
  priorMonths.forEach((monthStr, monthIndex) => {
    result[monthStr] = RAW_SEPTEMBER_SPECS.map(spec => buildRoomRecord(spec, monthIndex, monthStr));
  });

  // Month 8: 09 ก.ย. (Canonical Start Month / Fresh setup)
  const septRooms = RAW_SEPTEMBER_SPECS.map(spec => buildRoomRecord(spec, 8, '09 ก.ย.'));
  result['09 ก.ย.'] = septRooms;

  // Month 9+: 10 ต.ค., 11 พ.ย., 12 ธ.ค.
  // Directly take September's meter readings as the previous readings ("ข้อมูลเดือนก่อนของเดือนกันยาย")
  // and no arrears as default ("ไม่มีค้างจ่ายเป็น default")
  result['10 ต.ค.'] = septRooms.map(r => buildSubsequentMonthRoomRecord(r, '10 ต.ค.'));
  result['11 พ.ย.'] = septRooms.map(r => buildSubsequentMonthRoomRecord(r, '11 พ.ย.'));
  result['12 ธ.ค.'] = septRooms.map(r => buildSubsequentMonthRoomRecord(r, '12 ธ.ค.'));
