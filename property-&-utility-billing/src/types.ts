...
export interface LandlordConfig {
  ...
  carryOverArrearsDefault?: boolean; // ทบยอดค้างชำระไปงวดถัดไปโดยอัตโนมัติ (default: false ไม่มีค้างจ่ายเป็น default)
}
...
export interface LeaseAgreementData {
  id?: string;
  // Place & Date
  madeAt: string; // สัญญาทำขึ้น ณ
  agreementDate: string; // วันที่ทำสัญญา YYYY-MM-DD
  ...
  // Leased Property (ข้อ 1)
  roomNo: string;
  buildingName: string;
  propertyAddressNo: string; // เลขที่ตั้งอาคาร
  alley: string; // ซอย
  road: string; // ถนน
  ...
  // Term of Lease (ข้อ 2)
  leaseStartDate: string; // เริ่มสัญญา YYYY-MM-DD
  leaseEndDate: string; // สิ้นสุดสัญญา YYYY-MM-DD
  minimumYear: number; // ขั้นต่ำกี่ปี (default 1)
  
  // Rental Fee & Payment (ข้อ 3)
  monthlyRent: number;
  paymentDueDay: number; // ชำระภายในวันที่ 5
  bankName: string;
  bankAccountNo: string;
  bankAccountName: string;
  latePenaltyPerDay: number; // ค่าปรับวันละ 100 บาท
  
  // Security Deposit & Advance (ข้อ 4)
  securityDeposit: number; // เงินประกัน
  advanceRent: number; // ค่าเช่าล่วงหน้า
  refundDaysAfterExit: number; // คืนเงินประกันภายใน ... วันทำการ (default 15 หรือ 30)
  
  // Utilities (ข้อ 5)
  waterRatePerUnit: number; // ค่าน้ำหน่วยละ (default 28)
  electricityRatePerUnit: number; // ค่าไฟหน่วยละ (default 8)
  
  // Termination (ข้อ 8)
  terminationArrearsDays: number; // ค้างชำระเกินกว่า ... วัน (default 15 หรือ 30)
  vacateGraceDays: number; // ต้องย้ายออกภายใน ... วัน (default 7 หรือ 15)
  
  // Signatures
  lessorSignName: string;
  lesseeSignName: string;
  witness1Name: string;
  witness2Name: string;
  
  // Inventory Checklist (Page 4)
  inventory: LeaseInventoryItem[];
}
